...
This document's tables, figures and equations carry an S prefix, and the main
paper refers to them by those numbers. References without one point outward:
"Table 4" and "Fig. 1" are the manuscript's, as is "Section IV-A", while
"Table S6" and "Section S-III" point within this document.
...
**TABLE S2.**Hyperparameters, in the form the code sets them. Section III-C of the main paper summarizes this protocol in prose; this table gives the same settings in the names the code uses, together with the learning-rate search range behind them.
...
The hard-coded constant has been replaced by a recorded one: each training script now writes the learning rate it actually used, and every rebuild path reads it back, raising an error rather than guessing if the record is absent. The correction is verifiable — under the recorded rate the rebuild's per-epoch training curve is byte-identical to the training run of record, which the 3 × 10⁻⁴ rebuild demonstrably was not. M2's rows in both external evaluations were then re-measured. Its external specificity moves from 91.3% to **86.0%** and its generalization gap from −6.7 to **−1.4** points; on the synthetic proxy its accuracy moves from 0.623 to 0.633 and its ROC-AUC from 0.788 to 0.794. No qualitative claim turns on the correction: M2 retains the highest Split C specificity point estimate and the only negative gap. That it is the best-generalizing model was a separate claim, and Section IV-D withdrew it on the evidence of a second capture condition. The superseded values are preserved alongside the current ones in the archived results.
...
**A fourth defect, in a path that never checked itself.** While building the attribution audit of Section S-I-J we found that all three Grad-CAM scripts obtained the backbone by calling its constructor directly, which returns a module in training mode, and set only the classification head to evaluation mode. The backbone's 49 batch-normalization layers therefore ran on batch-of-one statistics and overwrote their running averages on every image. This was invisible for as long as nobody asked the scripts for a number that could be checked: the heatmaps looked plausible. It surfaced immediately once a script printed its external specificity alongside the heatmaps and that figure read 0.16 against the 0.807 of record. Section S-I-J states the consequence for Fig. S10; the scripts now force evaluation mode and the quantitative audit asserts its accuracy against the value of record before reporting anything.
...
One definition governs everything in this section. "Leakage-free", here and throughout, means free of **product-identity** leakage: Split B guarantees that no perceptual-hash cluster of near-duplicate photographs straddles a partition or a cross-validation fold. It makes no claim about acquisition. Because every counterfeit-labeled image in the pool was produced by one capture pipeline and every authentic-labeled image by another (Section IV-A), *no* partition of this pool can place a capture process on only one side of a fold, and grouped cross-validation inherits the confound in every fold at full strength. The numbers below are therefore the best in-distribution estimates obtainable on this data and are still, in the sense Section IV-A makes precise, measurements of the acquisition process. Readers should not read Split B as a corrected evaluation; it corrects one of the two problems.
...
Counting directly: **7 of the 76 Split A test images belong to a product-identity group that also appears in Split A's training partition**, a direct exposure rate of 9.2%. Those seven are the only images any model could classify correctly by recognizing a training photograph rather than by generalizing, so 9.2 points is the most that *recognition* can contribute. The count follows from the split alone, involving no model and no run, so it is immune to the sampling-variance objection that limits the measured deltas — which duly fall below it (+0.2 to +6.8 points).
...
**M3 is at chance (AUC 0.503, 95% CI [0.438, 0.569], MCC −0.041), and this cross-validates the attribution finding of Section S-I-J.** M3's comparatively high external specificity is best explained by a surround-based rule (Section S-I-J); such a rule provides no signal for separating a photograph from a perturbed copy of *the same photograph*, since the surround is identical in both. Chance performance is what that explanation predicts, obtained here from a completely independent evaluation set.
...
**In-distribution (M4, Split B), n = 22.** Nine of 22 (41%) place attribution mass on packaging-relevant regions — logo, brand header, printed text — seven (32%) place it on incidental regions such as background, corners or backdrop, and six (27%) are mixed. Split by outcome, the pattern is mild but in the expected direction: of 15 correct predictions, 7 attend to the product and 3 to incidental regions; of 7 errors, 4 attend to incidental regions and 2 to the product. In-distribution, then, the attribution is unremarkable — mostly on the product, more often incidental when the model is wrong.

**External (M4 and M3, Split C), n = 20 each.** The external sets behave completely differently and, on this method, the two models behave *identically* — a reading the occlusion analysis later in this section sustains for M3 and withdraws for M4:
...
The separation is total, in both models, with no exceptions in 40 heatmaps. Read with the target-class convention above, it says something specific and damaging: **on external images the attribution mass supporting "authentic" concentrates in the background, and the mass supporting "counterfeit" on the product.** A second, annotation-free method reported below confirms three quarters of that pattern and contradicts the remaining quarter, so the claim the paper finally makes is narrower than this one. When either model gets an external image right, it is not because it recognized authentic packaging; it is responding to the photographic surround. When it gets one wrong, it is looking straight at the printed carton and concluding "counterfeit".

This supersedes and generalizes an earlier reading. A previous version of this analysis made a similar argument about M3 alone, from five heatmaps, and treated M4's external attribution as evidence that the model was making a "spatially sensible decision calibrated to the wrong cue". With the complete set and corrected heatmaps, the finding is not specific to M3 and is not a point in M4's favor: on this method both frozen backbones do the same thing, and the product-focused attribution on M4's errors is the *failure* half of the pattern, not a redeeming feature. What survives the second method is the M3 half and the errors of both.

It is consistent with the quantitative result of the previous subsection, though we put that more weakly than an earlier version of this paper did. Split C is 100% authentic, so the model predicts "authentic" on most of it, and a surround-driven prediction should push attribution mass away from the center; the normalized model's border mass is 0.655 against a uniform reference of 0.642. That is in the predicted direction but it is also, in absolute terms, indistinguishable from uniform, so it corroborates the categorization only weakly and cannot be offered as independent confirmation of it.
...
The result does not flatter the correction, and is the more interesting for it. **Normalization does not move attribution onto the product; it makes attribution diffuse.** The un-normalized model is strongly center-concentrated — 0.563 of its attribution mass in a center box covering 0.161 of the frame, a 3.5-fold over-representation on exactly the region where a centered product sits — and it classifies 6% of external images correctly. The normalized model distributes attribution almost exactly uniformly (border 0.655 against a uniform 0.642; center 0.165 against 0.161) and classifies 80.7% correctly. Across these two conditions, spatial concentration of attribution **anti-correlates** with external specificity.
...
One conclusion survives under either reading, and it is a caution against a common practice: **a visually convincing, product-centered Grad-CAM map is not evidence that a model has learned the intended task.** That claim rests on the *un-normalized* model, whose input is unaltered and whose attribution is therefore not in question — it is strongly product-centered, and it classifies 6% of external images correctly. The model whose attribution looks most like it is reading the product is the one that fails externally.

**A second attribution method, and the one place it disagrees.** Everything above rests on Grad-CAM: one method, one annotator, forty sampled maps, and a method this subsection has just argued is degraded by the bottleneck it runs behind. `modeling/occlusion_sensitivity.py` puts the same question to a method that shares none of those weaknesses. It slides a 64 px occluder over the network's 224 × 224 input on a 6 × 6 grid and records how far P(authentic) falls when each region is hidden; a region whose occlusion costs the model its "authentic" answer is a region that answer was resting on. The positive part of that surface is normalized to unit mass and summarized with the border statistic used above, so the two methods are comparable by construction. It needs no annotation, no categorization and no sampling, so it runs on all 150 external images for each model, from the persisted production checkpoints — and each model reproduces its Split C specificity of record, 0.773 and 0.807, before any attribution number is reported. Where it and Grad-CAM disagree, on M4's correct external answers, neither settles the case; the region-substitution experiment of the main paper's Table 5 does, and in Grad-CAM's direction. Occlusion asks how much hiding one 64 px patch moves the score, which a decision spread over half the frame answers weakly wherever that half is; substitution destroys a whole region at once and is not subject to that.
...
We report the disagreement rather than choose a winner, and we read the paper's claim down to what both methods support. Three things bear on which to weight. The occlusion measurement covers every external image rather than a seeded sample of ten, and it is causal rather than gradient-based, which are reasons to prefer it. Grad-CAM here runs behind the 128 px bottleneck this subsection has already argued degrades it, which is a reason to distrust the maps. And the two do not measure the same quantity: Grad-CAM weights a feature map by the gradient of the predicted-class logit, occlusion measures what removing a region actually costs, and a model can carry a gradient signature over the surround without a causal dependence on it. What we will not do is keep the stronger claim because one of the two methods supports it. On the evidence in this subsection the surround dependence stands for M3 and for both models' errors and is unresolved for M4's correct external answers, occlusion being a local test that a decision spread over a large region answers weakly. Section IV-E settles it with a global one: region substitution finds both backbones dependent on the surround, which is the direction the categorization read and the direction we report, and it is the reading that the two backbones behave *differently* that is withdrawn.

The content-aware measure that would settle it is still the one Section V-C names — attribution mass inside an annotated product box rather than inside a radial ring, which distinguishes "away from the center" from "not on the product" for images whose product is off-center. `scripts/23_build_product_box_tool.py` and `modeling/attribution_in_box.py` implement it end to end and are committed; what they need is the annotation pass, which has not been done.
...
The shape of Table S7 is worth stating plainly against Section IV-D. In-distribution, these models make few errors and most of those errors are explicable. Externally, two of them were wrong on every single image. Both statements are true simultaneously, and a study reporting only the first would be describing a system that does not work as one that works well.
...
| M4 EfficientNet-B0 | A | 18 | 13 | 0.082 | 0.974 |
| M4 EfficientNet-B0 | B | 18 | 14 | 0.045 | 0.987 |

### M. Calibration of the probability outputs

Everything above treats the models as decision rules at a fixed threshold. Any deployment as a triage tool would instead read the probability, so calibration is the property that decides whether the score can be thresholded at all. Table S10 quantifies it from the persisted per-image scores, on the leakage-free test partition and both acquisition-shift conditions, with no retraining.

**TABLE S10.**Calibration on the leakage-free test partition and both acquisition-shift conditions, computed from the persisted per-image scores. Brier is the mean squared error of the predicted probability. ECE is expected calibration error over ten equal-width confidence bins; MCE is the worst single bin. Conf is mean confidence in the predicted class; conf − acc is positive for overconfidence. Splits C and D are authentic-only, so what is computed on them is a specificity and their calibration is measured against one class; see the caveat below.
...
**M4 is the only model whose calibration survives both capture shifts** (0.061 → 0.098 → 0.055), which is consistent with its accuracy behavior in Table 4 and, like that result, says nothing about whether it is reading packaging — Section S-I-J applies unchanged.

One caveat bounds all of this. Splits C and D contain only authentic images, so accuracy on them is a specificity and the calibration statistics are measured against a single class; ECE and accuracy are therefore not independent quantities there in the way they are on Splits A and B, and the external rows should be read as a description of how confidently each model commits its false positives rather than as a full calibration curve. The balanced external test of Section IV-C is the two-class external set on which calibration would be measurable in the ordinary sense; it was scored once for the metrics of Table 3 and its calibration was not computed.
...
| Group | Res. | Bright. | Comp. | White bal. | Split B accuracy | Split C specificity |
|---|---|---|---|---|---|---|
| **(i)** | | | | | 0.919 | 0.087 |
| (i) | ✓ | | | | 0.932 | 0.220 |
| (i) | | ✓ | | | 0.919 | 0.273 |
...
**Each of resolution, brightness and compression helps alone, and each helps modestly.** Individually they take external specificity from 5–9% to 22.0%, 27.3% and 12.7% respectively — real effects, none sufficient.
...
The conclusion survives, more firmly than before. Gray-world normalization applied alone leaves external specificity at 0.067, essentially the unnormalized baseline. Added to the production three-way operator it moves external specificity from 0.820 to 0.780. Added to the two-way operator — the comparison the original experiment attempted — it moves 0.500 to 0.347, a decrement four times larger than the one originally reported. Both combinations point the same way, and the axis buys nothing in either.
...
| Condition | Operators | Split B test accuracy | Split C specificity |
|---|---|---|---|
...
> **FIGURE S11.** `paper/figures/fig10_ablation.pdf` — (a) M4 within-run ablation of the three retained axes. (b) Change in external specificity under two-way and three-way normalization, per model.
...
The final column of Table S13 does not reproduce Table 4, and the difference is expected rather than a discrepancy to reconcile: Table 4 reports the production pipeline, Table S13 reports standalone single-run ablation scripts. For M3 the two read 0.773 and 0.813 externally, for M4 0.807 and 0.780, and for M2 0.860 and 0.913. Only Table 4's column is the result of record. We report both rather than silently dropping the ablation's absolute values, because the within-run deltas those runs measure are the ablation's actual claim and are unaffected.

Two further caveats attach to the M2 rows specifically. Both ablation scripts trained M2 at 3 × 10⁻⁴ — the same stale constant described in Section S-I-G — so their absolute values are not comparable to M2's production numbers in Table 4 (which use the recorded 1 × 10⁻³). Because baseline and normalized conditions within a single run share that learning rate, the *within-run* comparison each row reports is unaffected, which is the claim the ablation makes; the scripts now read the recorded rate, so a re-run would refresh the absolute values.
...
A 96-dimensional color histogram has almost nothing to work with once mean brightness is standardized away, because — per the exact linear logit decomposition of Section IV-A — mean brightness, expressed through the near-white bin, *was* essentially its entire decision function. Fig. S12 shows both halves of that decomposition: 93 of the 96 coefficients lie within ±0.35 of zero, and the three near-white bins carry mean |φ| of 0.079–0.082 on the Split B test partition against ≤ 0.002 for the remaining 93.

> **FIGURE S12.** `paper/figures/fig12_model1_attribution.pdf` — (a) M1's logistic-regression coefficients across the 32 intensity bins of each RGB channel; the near-white bin dominates all three channels. (b) The eight features with the largest mean absolute logit contribution on the Split B test partition. Attribution is exact for this model, not sampled. Removing the confound does not unlock latent signal in this model, because there was no latent signal to unlock. We therefore report M1 as a clean negative result: **on this benchmark, a classical color-statistics baseline is structurally incapable of the intended task, and its 83.8% accuracy is a measurement of the confound rather than of packaging authenticity.** We considered enlarging its feature set (texture descriptors, color moments, edge statistics) and decided against it: the model shows zero external signal across every condition tested in this study, in both external evaluations, and a larger hand-crafted feature set is unlikely to overturn a pattern that consistent. The negative result is the useful finding.

This has an uncomfortable implication, and it is worth being precise about its reach. On *this* dataset it is direct, and the header-only oracle of Table 2 makes it exact rather than suggestive: three header fields with no pixel decoded reach 100% on the leakage-free partition, and a color histogram reaches 83.8%. There is no residual on this pool that a convolutional network is needed to explain. M1's 83.8% is the more conservative statement of the same point and the one we would defend if the oracle were disputed, but the oracle is the tighter bound. Beyond this dataset the implication is conditional rather than established: it applies to any collection in which the two classes were produced by different pipelines, which Section II-A finds is the majority of the located prior work — [26] edits authentic images to create its counterfeit class, [27] generates it — but we have not audited those collections and do not claim their reported figures are confounded. We claim they are unaudited, and that the audit is cheap.

### Q. The architectural ablation implicit in M2

M2 carries an architectural observation that is worth separating from the confound results. It scores 0.865 on the leakage-free split — no statistically significant difference was detected between it and any other model in the roster on the shared internal test partition (Table S5) — and it has the highest external specificity point estimate after correction (0.860, Table 4), with the only negative generalization gap anywhere in this study. It reaches that with 23,938 parameters and no pretraining.

The comparison this licenses is a narrow one, and we state it narrowly. A `flatten → dense(128)` head on this trunk would add roughly 6.4 M parameters, some 267 times the rest of the network, trained on 357 images. Nothing in these results suggests that capacity would buy anything: the accuracy ceiling on this pool is set by the confound (Section IV-A), not by model capacity. Parameter count is the wrong axis on data like this.

What we no longer claim is that M2 generalizes best. On Split C alone its point estimate is highest, by a clear margin. On the second capture condition of Section IV-D it collapses to 0.463 while both frozen backbones hold, which makes its Split C advantage a property of that particular capture condition rather than of the model. The defensible version of the architectural observation is therefore weaker and more interesting: a 23,938-parameter network from scratch can match far larger pretrained models in-distribution and on one external set, and still fail to carry that to a second — so neither in-distribution accuracy nor a single external evaluation predicts which model to deploy.

### R. Are the three constants load-bearing?

Eqs. (5)–(7) of the main paper fix three constants: a 128 px short side, a target mean of 0.5 and JPEG quality 40. Only the first has a justification derived from the training distribution alone. A reviewer may reasonably suspect that a recovery from 6.0% to 80.7% rests on three fortunate choices, so we varied each one around its production value, on M4, inside a single script execution (Table S14).
...
| Varied | Value | Split B test accuracy | Split C specificity |
|---|---|---|---|
...
One reading of the diffuse attribution reported in Section S-I-J is that the bottleneck simply destroys the fine spatial structure a genuine packaging classifier would need, leaving a model that cannot localize anything. This sweep is the direct test of that reading, and it does not support it. If the 128 px cap were already past the point where authentication-relevant detail survives, tightening it to 96 px should cost external specificity; instead 96 px is the best external condition in this sweep (0.873), for 1.4 points of in-distribution accuracy. Detail is being removed, and removing more of it continues to help up to the limit tested. What the sweep cannot settle is the complementary possibility — that the intended task needs detail this pipeline never had access to, in which case both the confound and the signal are being suppressed together and only the confound is being missed. Distinguishing those requires a source whose two classes share an acquisition pipeline, which Section V-D lists as the outstanding acquisition.

### S. Deriving the axes without the external set

Sections S-I-N to S-I-X establish what the correction recovers, that it is robust to its constants and that it is sensitive to its composition order, but not that it could have been *found* without the external set. The axes were originally identified by comparing the Kaggle pool against Split C (Table 1), which is target-distribution information no practitioner deploying a model would possess. This section removes that dependency, because it is the most serious objection to the correction and, until now, an untested one.

The procedure is the audit of Section IV-A turned into an axis-selection rule and confined to the training data. Using the **Split B training partition alone** — 357 images, 336 product groups, with grouped 5-fold cross-validation *inside* that partition so that neither the validation partition nor the test partition nor Split C is consulted at any point — we fit a one-feature classifier to each acquisition statistic a practitioner could compute from their own files, and normalize every axis whose balanced accuracy clears a threshold declared in advance (0.65). Table S15 gives the result.
...
**Type A — acquisition-statistic confound.** The classes differ in format, resolution, compression, brightness or aspect ratio because they were captured or encoded by different processes. *Detection:* the provenance audit of Section IV-A — fit the intended classifier to metadata alone. *Instance:* the Kaggle dataset, audit accuracy 1.000, with even aspect ratio alone at 0.803. *Cost when undetected:* in-distribution 0.919, external 0.060. *Partial repair:* the label-free normalization of Section III-F, architecture-dependent.
...
The harness reproduces the values of record. At seed 42 the normalized condition returns M2 0.865 / 0.860 / 0.463, M3 0.932 / 0.773 / 0.725 and M4 0.919 / 0.807 / 0.832 on Split B, C and D — the published numbers exactly, from the pipeline as it now stands. The table below gives the mean ± sample standard deviation across the five seeds (Split C and D columns are specificities; the main paper's Table 4 is this table's seed-42 draw; M1 is omitted because a convex fit with a deterministic solver and no augmentation is identical under every seed). This table is deliberately uncaptioned so that it does not advance the supplement's table counter; the main paper cites it as Section S-I-U.
...
> **FIGURE S13.** `paper/figures/fig08_external_generalisation.pdf` — In-distribution authentic accuracy against external specificity before and after three-way normalization, per model, for the run of record. It plots Table 4 of the main paper; M1 bypasses the normalized pipeline by design, so its two Split C bars are the same 0.0% measurement shown twice.

Fig. S13 plots the run of record behind those means.

**The archived pre-normalization run, and why it is superseded rather than co-equal.** Earlier versions of this paper reported the baseline column of the main paper's Table 4 from an archived production run made immediately before three-way normalization became the default, which returned 0/150 for M1 and M2, 104/150 for M3 and 5/150 for M4. Those figures are not lost and never were: the run's own log, with its full per-epoch training curves, is committed at `modeling/results/split_c_eval_log.txt` and has been present since the repository's first commit and in every archived release. `paper/scripts/external_intervals.py` now parses the counts from that log rather than carrying them as constants, so they are traceable rather than transcribed.

What could not previously be explained was why the current harness returns different values for the same nominal condition. It can be now, and the explanation is that the archived run was produced by a pipeline carrying the two defects of Section S-I-G, both since fixed. Comparing the archived per-epoch curves against the current harness at seed 42, under nominally identical seeds and hyperparameters:

| Model | Archived epoch 0 | Current epoch 0 | Divergence | Defect that accounts for it |
|---|---|---|---|---|
| M2 CNN | 0.6157 / 0.675 | 0.5777 / 0.709 | large | learning rate hard-coded at 3 x 10^-4 |
| M3 MobileNetV3 | 0.4661 / 0.836 | 0.4680 / 0.820 | small | unseeded augmented feature passes |
| M4 EfficientNet-B0 | 0.3821 / 0.871 | 0.3788 / 0.867 | small | unseeded augmented feature passes |

Train loss and train accuracy at the first epoch, before early stopping can differ. The pattern separates the models exactly along the line the two defects predict, and the two groups are disjoint. M2 is the one model whose learning rate the pre-fix external-evaluation script hard-coded, and it is the one model whose trajectory diverges immediately and substantially. M3 and M4 are the two models that consume the K = 3 cached augmented feature passes, which were unseeded before the fix, and their trajectories diverge by an order of magnitude less — a perturbation of the training features rather than of the optimizer. Neither group shows the other's signature.

This also explains the one observation that a seed effect alone does not. M4's archived 5/150 lies *below* all five re-derived seeds, which span 9/150 to 20/150. The seed sweep varies the training seed while holding the now-seeded augmentation scheme fixed, so it does not span the source of variation the archived run was subject to; a draw from outside its range is what one would expect, not an anomaly.

We therefore treat the archived run as **superseded** rather than as a competing measurement, and the main paper's Table 4 reports both of its columns from the current deterministic pipeline at seed 42, so that the baseline and normalized columns differ only by the operator of Eq. (8). The archived figures are retained here as an audit trail and are quoted nowhere as a result. Nothing in the argument moves: every measurement of the baseline condition, archived or current, agrees that it is near zero for M1, M2 and M4 and around 0.7 for M3, and every measurement of the corrected pipeline agrees that it is not.
...
Two further limits are worth stating. The exposed subset is 28 images, which is every one the pool offers and still small; the ±2.9-point interval on that subset is what the data support and no tighter statement should be read into the point estimate of 0.0. And this measures near-duplicate leakage as `pHash` clustering defines it (Section III-A). Two dissimilar photographs of the same physical package would not be grouped, so both the exposure count and this experiment address near-duplicate leakage specifically, not every sense in which two images might share a product.

### W. The provenance audit on datasets other than the case study

Section IV-A runs the audit on the case-study pool and summarizes what follows here in prose. This section is the full record behind that summary: seven archives across four application areas, scored from public file listings alone, and then the second pharmaceutical dataset in the detail that establishes the audit's false-negative mode. Every cross-domain number the main paper quotes is taken from Table S18 or Table S20 below, and the caveats on both apply to it unchanged. The two are reported in that order because the first tests whether the audit discriminates at all and the second tests what it misses.

The capture statistics behind Table 1 of the main paper are plotted here rather than there, for space.

> **FIGURE S14.** `paper/figures/fig03_capture_confound.pdf` — Distributions of the three confounded statistics. (a) Brightness: violin plots with group means labeled. (b) Short-side resolution, log scale, with medians. (c) File size, log scale, with means. The external set lies outside the range of both training classes on all three axes.

**The audit across application areas.** A survey is what would test the wider claim, and its first step costs nothing: Kaggle's public file-listing endpoint returns the path and encoded size of every file in a dataset without authentication, the path carries the class in the folder-per-class layout these archives use, and the extension carries the format — so two of the four audit features can be fitted **as the publisher shipped it, without downloading a single image** (Table S18).

**TABLE S18.**The provenance audit run from public file listings alone, no images downloaded. Balanced accuracy under stratified 5-fold cross-validation; chance is 0.500. Format is the one-hot file extension, size is log encoded bytes. Ext-rule is the accuracy of the single deterministic rule "extension predicts class". Listings are paginated and the endpoint stops at 2,400 files, so large archives are sampled in listing order; entries are files, not unique images, which is why the Kaggle archive lists 2,224 for 661 unique images — it re-lists all of them across its bundled `train`/`val`/`test` folders (Section III-A of the main paper). This is a pilot audit of seven archives chosen for accessibility, not a prevalence estimate over a defined sampling frame.
...
Table S19 reads the resulting scores back as guidance, and three things follow from the rows themselves, of which the last two are the useful ones.

**The positive control passes, by an independent route.** The case-study dataset returns 1.000 on format and on the deterministic extension rule from its *public listing metadata alone* — no pixels, no download, no local pipeline — where Section IV-A reached the same conclusion from the archive on disk. The paper's central empirical claim is thus checked twice on entirely different inputs.
...
**The negative control reveals a false-positive mode, and it changes how the audit's output should be read.** Genuine and forged signatures in the BHSig260 corpus are written on the same paper and digitized by one procedure, so there is no acquisition asymmetry to find, and format duly returns exactly 0.500. Encoded size returns **0.843**, most likely because forged signatures differ in stroke complexity and so in ink coverage, which drives the compressed size of a bitonal scan. The audit therefore answers a narrower question than "is this dataset confounded" — it answers "can a trivial statistic separate your classes", and the researcher must then establish whether that statistic is an acquisition artifact or a property of the objects. A high score is a reason to investigate, not a verdict, and format is the more specific axis because storage format is never a property of the photographed object (Section S-I-T, Type E).

---

**TABLE S19.**How to read an audit score. These are the values this study measured, not validated thresholds: with five scorable datasets there is no basis for a false-positive rate, and the rows below are an aid to interpretation rather than a decision rule. The **format** axis is the discriminating one, because storage format is never a property of the photographed object, whereas encoded size, resolution and aspect ratio mix acquisition with content and can score high for reasons that are not confounds.

| Observed score | Dataset | What it supported |
|---|---|---|
| 0.500 format, 0.843 size | BHSig260 signatures | One acquisition procedure for both classes, and the audit's false-positive mode: the size score comes from a content difference, so a high score on an ambiguous axis alone is not evidence of a confound |
| 0.577 format, 0.562 overall | second generated-image set | Near chance; no acquisition asymmetry these features detect |
| 0.500 format, 0.717 overall | Roboflow archive | An archive that was in fact totally confounded, its format signal erased by the publisher's own resizing and re-encoding. Investigate a middling score rather than reading it as clearance, and treat a normalized archive as unauditable by this method |
| 1.000 format | Kaggle pool, one generated-image set | Acquisition determines the label. No in-distribution accuracy on the dataset is interpretable |

**The same audit on a second, independently published dataset.**

One dataset cannot establish a general claim, so we ran the identical procedure against the other public authentic/counterfeit pharmaceutical image dataset we were able to obtain — the Roboflow source of Section III-A, a different publisher, a different country of origin and a different class-construction method. Both are audited **as shipped**, before any filtering of ours, under grouped 5-fold cross-validation on rotation-canonical pHash clusters so that a publisher's own augmented copies cannot straddle a fold. Balanced accuracy is reported because both are heavily imbalanced as distributed; 0.5 means metadata carries nothing about the label (Table S20).

**TABLE S20.**The provenance-confound audit on two independently published datasets, as shipped. Grouped 5-fold cross-validated balanced accuracy of a logistic regression on the named metadata alone — no pixel content. Chance is 0.500.
...
On the Kaggle dataset the audit is decisive on every axis, including one we had not anticipated: **aspect ratio alone reaches 0.803**, because screen captures inherit display proportions that photographs do not. Such a confound is not a single leak to be plugged; it is present redundantly in every statistic acquisition touches, which is why Section S-I-N finds three normalization axes each contributing separately.

On the Roboflow dataset the audit largely **fails to fire** — and that dataset is nonetheless confounded in the most extreme way documented here: 57/57 of its counterfeit-labeled source images are advisory-bulletin graphics carrying the ground-truth word rendered in the pixels, against 263/263 clean product photographs labeled authentic (Section III-A). Metadata registers only a partial trace (0.717 from file size, which sees that bulletin collages compress differently) and nothing from format, resolution or aspect ratio.

The reason is a trap rather than a limitation of one dataset. **The publisher normalized the archive**, resizing every image to 640 × 640 and re-encoding to a single format before release, destroying the acquisition *tells* without touching the acquisition *confound*. A dataset tidied for distribution therefore looks cleaner under this audit than a raw one while being no less confounded. The audit is accordingly a **necessary but not sufficient** screen, to be paired with the two checks that did catch this: a content pass over a sample of each class (Section S-I-A) and cross-source near-duplicate hashing (Section III-A).

### X. Composition order of the three operators

Section IV-E established that the composed operator of Eq. (8) works. This section decomposes it: which axes matter, whether they are complementary, whether the effect is architecture-dependent, what happens to a model that has nothing but the shortcut, and whether the result depends on the two things the operator's definition leaves free — the magnitudes of its three constants and the order of composition.
...
**Brightness placement is a second-order effect.** Within the three sound orderings, moving the brightness rescale changes external specificity by 6 points (0.820 → 0.880) against the 28-point gap between groups — the distinction of Section S-I-R in another guise: the bottleneck operators are position-sensitive, the location-shifting one is not.

**The in-distribution ranking is not merely uninformative here; it is inverted.** The ordering with the highest Split B accuracy of all six — B, C, R at 0.946 — is the ordering with the lowest external specificity of all six, 0.380. A practitioner selecting the composition order the ordinary way, by held-out accuracy on their own data, would have chosen the worst of the six available options, and would have watched a 2.7-point in-distribution spread (0.919–0.946) conceal a 50-point external one (0.380–0.880). This is the sharpest demonstration in the study of what a confounded in-distribution partition is worth, precisely because nothing else varies: the same three operators, the same constants, the same model, the same data, reordered.
...
The consequence generalizes past this pipeline. Composition order is normally left implicit in a preprocessing description; on this evidence it deserves the treatment of a hyperparameter — reported, and not chosen in-distribution. Any pipeline composing two or more information-destroying operators should expect the same sensitivity, since the argument is about resampling and quantization rather than this dataset.

### Y. How much depends on the near-duplicate threshold?

Section III-A clusters at Hamming distance ≤ 8 on a 64-bit rotation-canonical perceptual hash, and that threshold was fixed before any model ran. Why 8 is a fair question, so we swept it. Re-clustering costs nothing: the hashes are already computed, no image is decoded, and the fixed Split A and Split B assignments can be checked against whatever groups each threshold produces.

**TABLE S22.**Sensitivity of the near-duplicate clustering to its Hamming threshold, over the 510-image modeling pool. "Mixed-label" counts clusters holding both authentic and counterfeit images — a check the clustering itself never sees, since it reads no labels. "Straddles" counts clusters with members in more than one partition of the fixed Split A and Split B assignments. Clustering the pool alone gives 482 groups at the production threshold where the pipeline records 480, because production clusters the Kaggle and Roboflow images together (Section III-A) and two Kaggle pairs are joined through a Roboflow intermediary; nothing in this table turns on the difference.

| Threshold | Clusters | Largest | Mixed-label | Straddles Split A | Straddles Split B |
|---|---|---|---|---|---|
| 0 (exact duplicates) | 507 | 2 | 0 | 2 | 0 |
| 2 | 495 | 2 | 0 | 7 | 0 |
| 4 | 490 | 2 | 0 | 7 | 0 |
| 6 | 486 | 3 | 0 | 8 | 0 |
| **8 (production)** | **482** | **3** | **0** | **9** | **0** |
| 10 | 472 | 4 | 0 | 13 | 6 |
| 12 | 444 | 10 | 3 | 18 | 17 |
| 16 | 260 | 183 | 10 | 19 | 28 |

Table S22 gives the sweep. Three readings follow, and the third is a limitation rather than a reassurance.

**The threshold is bounded above by evidence the clustering does not use.** Up to 10, no cluster mixes the two class labels; at 12 three do, and at 16 the largest single cluster holds 183 of the 510 images. Past 10, then, the hash is merging images the annotations call different, and the proxy has stopped tracking product identity. Eight sits inside that range with margin, which is the justification we can give for it: not that 8 is optimal, but that the clustering agrees with an external label there and stops agreeing shortly after.

**What the split comparison concludes does not depend on the threshold.** Countable Split A leakage runs from 2 clusters under exact matching to 18 at threshold 12 — 0.4% to 3.8% of clusters. Every value in that range is small beside the acquisition confound, so no admissible choice would have changed Section V-A. The same holds for Section S-I-V, whose 30 mates are the pairs at distance ≤ 8; a stricter threshold would shrink that set and a laxer one would contaminate it with different products.

**Split B's guarantee is a statement about distance ≤ 8, not about product identity.** At the production threshold no cluster straddles a Split B partition, which the pipeline asserts on every run. At 10 six would and at 12 seventeen. This is the limitation Section III-A states in words, quantified: perceptual-hash grouping is an operational proxy for product identity, and two dissimilar photographs of one physical package go ungrouped at every threshold this analysis can defend.

### Z. A paired comparison of the baseline and normalized conditions

Table 4 reports the correction as the distance between two Wilson intervals. Those describe two separate proportions; the quantity of interest is their difference, and because both conditions are evaluated on the *same* 150 external photographs the comparison is paired. `modeling/paired_external_test.py` runs it as one. It loads the persisted baseline and normalized checkpoints written by the seed sweep at each of the five seeds, replays them over Splits C and D, and keeps the per-image verdict rather than the count. The bootstrap resamples *images*, so both arms' verdicts on an image move together and the pairing survives resampling; this is the construction of Section S-I-V, used here for the same reason. Every replayed accuracy is asserted against the value the seed sweep recorded for that model, seed and condition before anything is written — the practice Section S-I-G argues for.

**TABLE S23.**Normalized minus baseline on the two external sets, five seeds (42–46). Accuracies are means over seeds. The difference and its 95% interval come from a bootstrap over images. Both external sets are authentic-only, so every figure is a specificity.

| Model | External set | Baseline | Normalized | Difference (percentage points) | McNemar *p*, pooled |
|---|---|---|---|---|---|
| M2 CNN | Split C (n = 150) | 0.051 | 0.909 | **+85.9 [+81.7, +89.7]** | < 10⁻¹⁵ |
| M2 CNN | Split D (n = 149) | 0.176 | 0.627 | **+45.1 [+39.9, +50.3]** | < 10⁻¹⁵ |
| M3 MobileNetV3 | Split C (n = 150) | 0.715 | 0.724 | +0.9 [−6.9, +8.8] | 0.688 |
| M3 MobileNetV3 | Split D (n = 149) | 0.644 | 0.685 | +4.0 [−2.7, +11.3] | 0.028 |
| M4 EfficientNet-B0 | Split C (n = 150) | 0.100 | 0.860 | **+76.0 [+69.9, +81.5]** | < 10⁻¹⁵ |
| M4 EfficientNet-B0 | Split D (n = 149) | 0.240 | 0.878 | **+63.8 [+56.9, +70.5]** | < 10⁻¹⁵ |

**The two conclusions the main paper draws survive the change of instrument, and are sharpened by it.** M2 and M4 gain 85.9 and 76.0 points on Split C with intervals nowhere near zero, so neither rests on Split C being only 150 images. M3 gains 0.9 points with an interval that spans zero symmetrically, which is the same verdict Section S-I-U reaches from the seed spread and Section IV-E reaches from overlapping Wilson intervals, now from a test that uses the pairing.

**The McNemar column should be read with its caveat, and it illustrates why the three kinds of uncertainty of Section III-B have to be kept apart.** It pools discordant pairs across the five seeds, which treats 745 verdicts as independent when they are 149 or 150 images seen five times by correlated runs. The pooling inflates every *n* and therefore deflates every *p*, and M3 on Split D is where that shows: McNemar returns 0.028 while the image bootstrap returns +4.0 [−2.7, +11.3], an interval containing zero. The bootstrap is the statistic we report, because it is the one whose resampling unit matches the design. A test that controls sampling uncertainty says nothing about training-run variability, and pooling seeds into it does not convert one into the other.

Two limits carry over from Section S-I-U. The baseline arm here is the five-seed mean rather than the seed-42 run that Table 4's baseline column reports, which is why M2's baseline reads 0.051 rather than 0.000 and M4's 0.100 rather than 0.060; Section S-I-U gives the per-seed values and no conclusion turns on it. And M1 is out of scope, having no checkpoint and never passing through the operator.

## S-II. Limitations

**The acquisition-shift experiment is authentic-only, so it measures no counterfeit recall.** Every image in conditions C and D is genuine packaging, so every number computed on them is a specificity — equivalently, one minus the false-positive rate — and nothing else. Counterfeit recall is measured elsewhere and on different terms: on the regulatory source in its native encodings (Section S-VIII) and, beside a specificity on the same footing, on the balanced external test (Table 3, Section S-IX). Neither is a recall under acquisition shift, which Section V-D records as unmeasured, and the synthetic proxy of Section S-I-B does not substitute for one.
...
**The general claim rests on a narrow evidence base.** Section I-A identifies asymmetric class sourcing as a mechanism producing provenance confounding, and makes no claim about how often it occurs. The direct evidence here is two datasets in one application area, an opportunistic screen of six further public archives of which three were scorable (Section S-I-W), and a structurally matching report from generated-image detection [30] that we did not produce. That is enough to motivate the mechanism, to justify the audit as a routine precaution, and to explain three independent observations of one signature; it is not enough to quantify how often the mechanism fires, and no number in this paper should be read as a prevalence estimate. Section V-B states what would falsify the claim.

**The correction's axes were originally chosen with knowledge of the external set, though they need not have been.** Eq. (8) of the main paper uses no label information, which is what licenses applying it to Split C; but the *choice* of resolution, brightness and compression was made after Table 1 showed those statistics separating the Kaggle pool from Split C, and that is target-distribution information a practitioner would not have. Section S-I-S addresses this directly: a train-only audit, confined to the Split B training partition with grouped cross-validation inside it, nominates all three axes without consulting any external data. The objection is therefore answered on this dataset. Two residual weaknesses remain. The selection threshold (0.65 balanced accuracy) is a judgement rather than a derived quantity. More fundamentally, no training-set procedure can nominate an axis that is confounded in the deployment distribution but not in the training distribution, so a train-only audit is a safeguard against the confound you have, not a guarantee against the one you have not seen.
...
**Cross-validating across sources or capture hardware is not possible on this data.** The strongest answer to a provenance confound would be a grouped cross-validation whose folds are acquisition pipelines rather than products, so that every fold is evaluated on a capture process it never saw. That design cannot be built here, and the reason is the confound itself: the counterfeit class exists in exactly one usable source. The only other public authentic/counterfeit pharmaceutical dataset we could obtain has a counterfeit class that is 57/57 advisory graphics carrying the ground-truth word in the pixels (Section III-A), which is unusable at any position in a fold; the acquisition-shift conditions are authentic-only, and the regulatory source of Section S-VIII holds 46 independent cases, too few to hold out as a training fold. A source-held-out fold would therefore hold no counterfeit images. The grouped cross-validation we do run decorrelates product identity and nothing else, and we do not claim otherwise; Section V-A states the consequence, which is that in-distribution machinery of any kind cannot see this defect.

**Split D does not vary the backdrop.** Section S-I-J identifies a surround-based cue as the basis of M3's correct external predictions and of both backbones' errors, Section IV-E extends it to both backbones' correct answers by region substitution, and Split C and Split D share the same staging. The second external set therefore cannot test that cue, and the stability of M3 and M4 across the two sets should not be read as evidence against it. An external set that varies the photographic setting — different surfaces, in-hand or in-shelf photography, uncontrolled backgrounds — is the evaluation this study most obviously lacks, and is a different requirement from the balanced set of Section S-IX, which shifts source while standardizing encoding.

**Two capture conditions, one archive, one product set.** This limitation was stated in an earlier version as a caution and has since been partly measured. Section IV-D adds a second capture condition, and it changed a headline claim: M2's post-correction specificity proved specific to Split C (0.860 → 0.463) while both frozen backbones held. What remains unmeasured is still substantial. Both conditions come from the same archive, the same 150 products and the same laboratory protocol, differing only in device and lighting — so the evidence supports "the correction transfers across a change of camera for pretrained backbones" and not "the correction transfers." Nothing in that experiment tests generalization across products, across countries, or to photographs taken by end users in the conditions a screening tool would actually face; the balanced test of Section IV-C shifts source but standardizes encoding and varies no photographic setting. No number in this paper should be read as establishing a generalization *rate*.
...
Table S24 reproduces every normalization condition executed in this study, annotated with the script execution that produced it, so that valid within-run comparisons are identifiable. Absolute values are not comparable across groups; see the caveat of Section S-I-X.

**TABLE S24.**All normalization conditions. Res = resolution bottleneck (Eq. 5); Bright = brightness rescale (Eq. 6); Comp = JPEG bottleneck (Eq. 7); WB = gray-world white balance.
...
Two caveats on exact reproduction. The external evaluations load persisted checkpoints while the ablation scripts predate that mechanism and rebuild, so their values are comparable within a run and not across runs (Section S-I-G). Results predating the augmentation-seeding fix are identified in their captions, and superseded results are archived rather than deleted.
```bash
...
python scripts/18_capture_method_stats.py          # per-image capture statistics (Table 1, Fig. S14)
...
python modeling/gradcam.py                         # in-distribution attribution audit
python modeling/gradcam_split_c.py                 # external attribution audit, M4
python modeling/gradcam_split_c_model3.py          # external attribution audit, M3
...
python modeling/region_substitution.py             # third: region substitution, Table 5
python modeling/eval_external_from_checkpoints.py  # conditions C and D from checkpoints

# 3b. The balanced external test (Section S-IX). Steps 30-31 hit the Commons
#     API and take about 20 min; step 32 rehashes the whole raw tree.
python scripts/30_harvest_balanced_authentic.py    # product-matched candidate harvest
python scripts/31_download_balanced_authentic.py   # bounded-width fetch, resumable
python scripts/32_build_balanced_external.py       # automated screens, independence, contact sheets
python scripts/record_balanced_review.py notes.txt # apply a subject-matter review
python scripts/33_apply_review_and_build.py        # select 46, ingest both classes, audit, lock
python modeling/eval_balanced_external.py          # Table 3; runs once
python modeling/eval_split_e.py                    # the counterfeit source in native encodings
python modeling/split_e_screen_sensitivity.py      # Table S28
...
python modeling/paired_external_test.py             # paired baseline-vs-normalized, Section S-I-Z
...
python paper/scripts/cross_domain_audit.py         # Table S18; fetches Kaggle listings
python paper/scripts/power_and_leakage_bound.py
python paper/scripts/phash_threshold_sweep.py      # Table S22
...
## S-VI. Appendix D — Image Sources Used by Prior Work

Section II-A of the main paper argues from this inventory and states its three observations; the tabulation is here for space.

**TABLE S26.** Image sources used by located prior work on pharmaceutical authentication. "Audit" asks whether the study *reports* any check that acquisition conditions are balanced across its two classes; "not reported" is a statement about the paper, not about what its authors did.
...
| Roboflow *Counterfeit_med_detection* [21] | Advisory bulletins plus product photographs | Class tracks document type, not authenticity (Section III-A) | — | — |

---

## S-VII. Appendix E — The Operator a Train-Only Audit Nominates

Section IV-E of the main paper reports this result and its two readings; the tabulation is here for space. Produced by `modeling/experiment_train_only_operator.py`.

**TABLE S27.** The reported operator against the one a train-only audit nominates. R = resolution cap, B = brightness rescale, W = gray-world white balance, C = JPEG bottleneck. The nominated operator is the one the rule of Section S-I-S selects using the Split B training partition alone, at a threshold fixed in advance; no external image informs it. Splits C and D are authentic-only, so those columns are specificities. Both conditions run inside one execution, so the within-run comparison is sound; absolute values are not comparable across runs, because this experiment rebuilds each model rather than loading a checkpoint (Section S-I-G) — M4's reported-operator row reads 0.820 here against the 0.807 of Table 4 for that reason, and reproduces the committed color-balance ablation exactly on both of its conditions.

| Model | Operator | Split B test | Split C | Split D |
|---|---|---|---|---|
| M2 CNN | reported, R→B→C | 0.865 | 129/150 = 0.860 | 69/149 = 0.463 |
| M2 CNN | nominated, R→B→W→C | 0.784 | 139/150 = **0.927** | 75/149 = **0.503** |
| M3 MobileNetV3 | reported, R→B→C | 0.932 | 116/150 = 0.773 | 108/149 = 0.725 |
| M3 MobileNetV3 | nominated, R→B→W→C | 0.932 | 123/150 = **0.820** | 117/149 = **0.785** |
| M4 EfficientNet-B0 | reported, R→B→C | 0.919 | 123/150 = 0.820 | 124/149 = 0.832 |
| M4 EfficientNet-B0 | nominated, R→B→W→C | 0.932 | 117/150 = 0.780 | 122/149 = 0.819 |


## S-VIII. Appendix F — The Counterfeit Source's Eligibility Screen, and What It Is Worth

The balanced external test's counterfeit class is drawn from 202 regulatory-alert
candidates by removing 52 of them (Split E; Section III-D, Section S-IX).
This appendix records that screen and the sensitivity analysis on it. The screen
is applied to all 150 surviving photographs, of which the 46 case
representatives entering Table 3 are a subset. The removals are judgements about what each frame shows, made by one
reviewer looking at each image, and several of the exclusion codes correlate
with the class by construction — only the counterfeit class carries alert
banners, and only the authentic class is carton artwork. A reviewer is entitled
to ask whether the reported recall is a property of the models or of the screen.
This appendix records the screen and answers that question.

### A. The codes

Each candidate carries a decision, a code and a written reason, published per
image alongside the data so the screen can be read and overruled without
re-deriving it.

| Code | n | What it removes |
|---|---|---|
| `X_BANNER_COMPOSITE` | 16 | A burned-in caption naming the contents as falsified — rendered text present on one class and absent from the other |
| `X_NOT_PRODUCT` | 8 | No medical product in the frame: the WHO logo, the WHO emblem as line art, laboratory thin-layer chromatography plates, a chromatogram |
| `X_TEXT_ONLY` | 8 | A rendered table of product details, with no photographic content |
| `X_NOT_PHOTOGRAPH` | 8 | Flat vector carton artwork supplied as a reference for the genuine label |
| `X_ANNOTATED` | 5 | Regulator markup — ellipses and arrows — drawn onto the image |
| `X_MIXED_CLASS` | 3 | One frame holding both an authentic and a counterfeit exemplar, unlabelable under a binary scheme |
| `X_LABEL_UNVERIFIED` | 3 | The source does not assert a class |
| `X_CLASS_TOO_SMALL` | 1 | The single surviving authentic photograph; one image is not a class |
| **retained** | **150** | |

`X_NOT_PHOTOGRAPH` is the code that decides the design. Eight of the nine
authentic candidates are artwork rather than photographs, so applying the screen
leaves one authentic image and Split E is reported falsified-only rather than as
a 150-against-1 two-class set.

### B. The screen does not produce the result

Every candidate labeled falsified by its source — including the ones the screen
removes — was scored once from the same checkpoints. Table S28 re-reads those
predictions under five screen settings, from a set holding only single
whole-product photographs to no screen at all.

**TABLE S28.** External counterfeit recall on the full 150-image regulatory source, in its native encodings, under five settings of the eligibility screen, with 95% case-clustered bootstrap intervals resampling whole regulatory cases. The predictions are identical across rows; only the subset read changes. S0 is the setting the 46 representatives of Table 3 are drawn from; Section S-IX-D relates this one-sided measurement to Table 3's two-sided one. S3 restores the frames carrying burned-in falsification captions and S4 restores everything, including the WHO logo and the laboratory plates, which is the no-screen bound rather than a defensible set.

| Setting | n | Cases | M1 | M2 | M3 | M4 |
|---|---|---|---|---|---|---|
| S1 whole-product photographs only | 117 | 34 | 0.991 [0.97, 1.00] | 0.573 [0.47, 0.68] | 0.513 [0.41, 0.61] | 0.573 [0.45, 0.70] |
| S2 minus multi-panel montages | 128 | 36 | 0.992 [0.97, 1.00] | 0.594 [0.50, 0.69] | 0.555 [0.45, 0.65] | 0.609 [0.48, 0.73] |
| **S0 as reported** | **150** | **46** | **0.967 [0.93, 0.99]** | **0.593 [0.50, 0.69]** | **0.553 [0.46, 0.64]** | **0.607 [0.49, 0.72]** |
| S3 plus banner composites | 166 | 53 | 0.886 [0.82, 0.94] | 0.584 [0.49, 0.67] | 0.524 [0.43, 0.61] | 0.578 [0.47, 0.68] |
| S4 no screen at all | 183 | 56 | 0.836 [0.76, 0.90] | 0.590 [0.50, 0.67] | 0.552 [0.47, 0.63] | 0.601 [0.50, 0.70] |

M2, M3 and M4 move by at most 0.06 across the whole range, and every interval
overlaps every other interval for the same model. M1 stays between 0.836 and
0.992; its decline as the loosest settings are added is the color-histogram
baseline responding to the large white text banners those settings restore,
which is consistent with what Section S-I-P reports about that model and
irrelevant to the conclusion drawn from it. **The screen removes frames that
should not be scored; it does not manufacture the number.**

What the table does not do is substitute for a second reviewer. It shows that
the result is insensitive to the screen's *aggregate* effect, not that any
individual call is right, and no inter-rater agreement was measured (Section V-D).

### C. Duplication

Split E was checked against every image in the project's raw tree — the Roboflow
and Kaggle modeling pool, Splits C and D, the iphone11pro archive and the
synthetic counterfeits, 7,081 images — by the rotation-canonical pHash procedure
of Section III-A at the same threshold of 8/64. (Section S-IX-C reports the same
check against 7,283 images: the reference tree grew by the 202 candidates
screened here, which were on disk by the time the balanced set was assembled.
Neither figure is a correction of the other.) **0 of 202 candidates matched
anything within the threshold**; the nearest approach was Hamming distance 10,
the same distance at which Split C's nearest approach to the modeling pool sits,
with median 18. Byte-level SHA-256 finds no exact duplicate among the retained
images. Four harvested candidates were caught as pool near-duplicates during
collection and discarded before the screen ran, and 27 byte-identical repeats
were removed within the harvest itself: the alert archives genuinely re-publish
photographs across related alerts, so this is a live risk rather than a
hypothetical one.

Eight near-duplicate pairs exist among the candidates, all within one regulatory
case and all among the carton-artwork images that `X_NOT_PHOTOGRAPH` removes, so
**no near-duplicate pair survives into the retained 150**.

## S-IX. Appendix G — The Balanced External Test: Construction, Screen and Audit

Section III-D states what the balanced external test is and why each of its
design decisions was forced. This appendix records the numbers behind those
decisions, the per-image screens, and the audit of the finished set.

### A. Why neither existing source could supply the authentic class

The balanced set needs an authentic class independent of the modeling pool,
independent of the counterfeit class's source, and **not separable from it by
...
**manufacturer artwork of what the genuine article should look like**, so the
archive that can supply a counterfeit class structurally cannot supply an
acquisition-matched authentic one. The second and third rows are the reason the
acquisition-shift conditions stay where they are: pairing either with the
regulatory photographs would have opened a class–acquisition gap *larger* than
the one that disqualified the artwork, reproducing this paper's headline defect
inside its own external validation.

### B. The harvest, and the screens applied to it

The authentic class was harvested from Wikimedia Commons, a third-party
photographic archive unrelated to any source in this study, carrying per-file
license and authorship. Candidates were sought by product: the 46 regulatory
alerts name their products in their titles, and those names — brand names,
INNs, and the drug class where an alert names a class rather than a product —
became 42 search terms, supplemented by five category trees walked to depth 2.

| Stage | Remaining | Removed, and why |
|---|---|---|
| Commons search and category walk | 3,568 | bitmap candidates; SVG excluded at the API |
| Pre-download filter | 788 | title or credit names a non-photograph; native short side < 300 px; per-term cap of 24 |
| Downloaded | 672 | 116 returned no thumbnail rendition |
| Automated screens | 449 | 173 outside the counterfeit brightness support [0.341, 0.714]; 50 flat artwork; 11 within-set near-duplicates (`A_NEAR_DUPLICATE_WITHIN`) |
| Independence | 438 | **0** matched the reference tree within the threshold |
| Subject-matter review | **174** | 99 `A_NOT_PRODUCT`, 69 `A_PERSON`, 38 `A_NOT_PHOTOGRAPH`, 28 `A_HISTORICAL`, 25 `A_NO_PACKAGE`, 5 `A_AMBIGUOUS` |
| Brightness-matched selection | **46** | across 29 distinct product search terms |

**The artwork rule is calibrated, not asserted.** The screen that matters most
is the one that separates photographs from flat carton artwork, since that is
the rule that removed 8 of 9 authentic candidates on the regulatory side. Rather
than choose a threshold by eye, it is fitted on labeled data this project
already holds: the 8 regulatory candidates the screen of Section S-VIII judged `X_NOT_PHOTOGRAPH` as
positives, and the 150 accepted regulatory photographs as negatives. Three
candidate axes are scored and the best single one is taken.

| Axis | Artwork range | Photograph p5–p95 | Youden J |
|---|---|---|---|
| **Near-white pixel fraction** | 0.365 – 0.490 | 0.000 – 0.116 | **0.980** |
| Quantized color count | 198 – 867 | 340 – 2230 | 0.620 |
| Mean gradient magnitude | 0.0197 – 0.0225 | 0.0107 – 0.0396 | 0.473 |

Only the near-white fraction separates the two classes; on the other two the
artwork range sits inside the photographs' central band. The rule is therefore
a single threshold, `near_white_frac > 0.320`, placed at the midpoint of the gap
the classes leave, and it catches **8 of 8** artwork images at **3 of 150**
false positives. A first version of this screen required all three axes at once
and caught **0 of 8** — a conjunction of one discriminating axis with two noise
axes excludes nothing while looking careful, and it is recorded here because a
screen that fires on nothing is the worst of the available failures.

**The subject-matter review is the screen no statistic replaces.** Commons,
searched by drug name, returns a great deal that is not a photograph of a
medicine package. The 438 eligible candidates were laid out on 13 numbered
contact sheets and each was given a decision, a code and a written reason:

| Code | n | What it removes |
|---|---|---|
| `A_NOT_PRODUCT` | 99 | No medical product in the frame: vehicles, buildings, landscapes, aircraft, consumer electronics, laboratory equipment, signage, forms |
| `A_PERSON` | 69 | A person's face or body is a subject of the frame. An anonymous hand holding a product does not trigger this; such a frame is judged on what the hand holds |
| `A_NOT_PHOTOGRAPH` | 38 | Drawings, engravings, paintings, molecular renderings, diagrams, micrographs, period advertising art |
| `A_HISTORICAL` | 28 | Antique or museum objects rather than current retail packages |
| `A_NO_PACKAGE` | 25 | No manufacturer packaging or labeled container: loose tablets, capsules or powder, unlabeled syringes and ampoules |
| `A_AMBIGUOUS` | 5 | Subject undeterminable, or the frame may show seized rather than legitimate product |
| **accepted** | **174** | |

**Who performed it, and what it decided.** This review was carried out by the AI
assistant named in the Acknowledgment, working to the codes above under the
author's protocol; it is not a human reviewer's pass and no inter-rater
agreement was measured. Its rubric is subject matter only — is this a
photograph, of a medicine package, of a current retail product — and it is an
eligibility screen, not a labeling step: every class label in the set comes
from its source. **It makes no authenticity judgement**, and could not: nothing visible in a photograph
establishes that the contents of a carton were made by the manufacturer named
on it. The authentic side of this set is therefore an *authentic-labeled*
reference class, not a verified one, and Section V-D states the consequence. It is published per image with its reason, and the
contact sheets are regenerated by committed code, so any call can be checked
and overruled. Section V-D records this as a limitation alongside the
corresponding one on the counterfeit side.

### C. Independence

The 449 candidates surviving the automated screens were checked against
**7,283 images** — every image under `data/raw`, covering the Roboflow and
Kaggle modeling pool, both acquisition-shift conditions, the iPhone 11 Pro
archive, the synthetic counterfeits and the regulatory photographs — by the
rotation-canonical pHash procedure of Section III-A at the same threshold of
8/64. **0 of 449 matched anything within the threshold.** The nearest approach
was Hamming distance 10, against an iPhone 11 Pro photograph — the same closest
approach that condition C and the regulatory candidates each recorded, which is
a useful coincidence: three independently assembled external sources, three
identical nearest approaches, none of them a match. Eleven within-set
near-duplicate pairs were found and one member of each removed.

### D. Ingestion, and what it does and does not standardize

Both classes pass through one pipeline: decode to RGB, resample the short side
to 448 px with Lanczos filtering, re-encode as JPEG at quality 92 with fixed
chroma subsampling and metadata stripped. Container format, encoder, encoder
settings and stored short side are therefore **standardized across the classes by
construction**, which holds the acquisition attributes of $A_{\mathrm{pure}}$ constant by design and
takes the size and resolution components of $A_{\mathrm{mix}}$ with them. 448 px was chosen as
the nearest round figure to the regulatory photographs' 439 px median short
side; the models resize to 224 px regardless, so the choice is close to
invisible to them and decisive for the audit.

Two consequences are worth stating rather than leaving to be discovered.
**First, recall on this set is not comparable to recall on the same photographs
in their native files.** Scored in their original encodings, all 150 regulatory
photographs return 0.967, 0.593, 0.553 and 0.607 for M1–M4 under case-clustered
intervals (Table S28's S0 row); the 46 re-encoded representatives in Table 3
return 0.957, 0.652, 0.435 and 0.587. The two are different measurements on
overlapping images and neither supersedes the other on its own terms — but
Table 3 is the counterfeit-side evidence of record, because it is the only one
reported beside a specificity measured on the same footing. **Second,
brightness cannot be equalized this way** without intervening on content, so it
is handled by matched selection instead: candidates outside the counterfeit
class's observed brightness support were screened out, and the final 46 chosen
by nearest-neighbour brightness matching to the 46 counterfeit frames.

### E. The audit of the finished set

Section III-E's procedure, run on the finished 92 images:

| Axis | Kind | Balanced accuracy | Reference |
|---|---|---|---|
| Container format | $A_{\mathrm{pure}}$ | — | Constant (JPEG) by construction; carries no information |
| Stored short side | $A_{\mathrm{pure}}$ | — | Constant (448 px) by construction |
| Encoded file size (log₁₀) | $A_{\mathrm{mix}}$ | 0.598 | chance 0.500 |
| Aspect ratio | $A_{\mathrm{mix}}$ | 0.435 | chance 0.500 |
| Mean brightness | $A_{\mathrm{mix}}$ | 0.413 | chance 0.500 |
| **All three jointly** | $A_{\mathrm{mix}}$ | **0.620** | chance 0.500; **the case-study pool scores 1.000** |

Between-class mean brightness differs by **+0.005**, against the +0.233 that
disqualified the regulatory artwork and the −0.396 that disqualified condition
C. Five-fold stratified cross-validation, logistic regression, seed 42.

**How to read 0.620.** It is above chance and the paper does not pretend
otherwise. Encoded file size is the residual axis, which is what fixing
resolution and encoder quality predicts: with those held constant, file size
tracks scene complexity, and photographs of seized product in field conditions
are not equally complex to photographs uploaded by hobbyists. What the audit
establishes is that a provenance shortcut is *available* at 0.620 rather than
at 1.000 — a diagnostic, not a clearance, exactly as Section IV-A says of the
audit generally. A model scoring 0.652 balanced accuracy on a set whose
acquisition variables score 0.620 has not been shown to be reading packaging;
it has been shown not to be doing much better than the shortcut. That reading
is stated in Section V-D and is one of the reasons no deployment claim is made.

### F. The lock

`data/metadata/balanced_external_provenance.csv` was written and hashed before
any model saw the set, and the evaluation ran once. The order was: training,
then preprocessing, then normalization, then the 0.5 decision threshold and the
model-selection rule, then the set, then a single scoring pass from the
persisted Split B checkpoints — the same checkpoints that produced Tables 4 and
5, loaded rather than retrained. Nothing about any model or its pipeline was
changed after Table 3 was read. Had anything been, this appendix would say so
and Table 3 would move to the exploratory tier.

The images themselves are not redistributed, for the same reason nothing else
in this project is: 140 of the 150 regulatory photographs are held under
metadata-only permission, and the Commons files carry per-file license terms
that vary. What is published is the manifest, the per-image screen record with
every decision and reason, the per-image predictions, and the five scripts that
rebuild the set from a reader's own fetch (Section S-V).
