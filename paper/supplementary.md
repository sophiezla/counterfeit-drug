...
"Table 6" and "Fig. 1" are the manuscript's, as is "Section VI-A", while
...
The content-aware measure that would settle it is still the one Section X names — attribution mass inside an annotated product box rather than inside a radial ring, which distinguishes "away from the center" from "not on the product" for images whose product is off-center. `scripts/23_build_product_box_tool.py` and `modeling/attribution_in_box.py` implement it end to end and are committed; what they need is the annotation pass, which has not been done.
...
**M4 is the only model whose calibration survives both capture shifts** (0.061 → 0.098 → 0.055), which is consistent with its accuracy behavior in Table 8 and, like that result, says nothing about whether it is reading packaging — Section S-I-J applies unchanged.
...
M2 carries an architectural observation that is worth separating from the confound results. It scores 0.865 on the leakage-free split — statistically indistinguishable from every other model in the roster (Table S3) — and it is the *best* model externally after correction (0.860, Table 6), with the only negative generalization gap anywhere in this study. It reaches that with 23,938 parameters and no pretraining.
...
> **FIGURE S13.** `paper/figures/fig08_external_generalisation.pdf` — In-distribution authentic accuracy against external accuracy before and after three-way normalization, per model, for the run of record. It plots Table 6 of the main paper; M1 bypasses the normalized pipeline by design, so its two Split C bars are the same 0.0% measurement shown twice.

Fig. S13 plots the run of record behind those means. The baseline rows above are re-derivations, not the archived pre-normalization run that Table 6's baseline column reports. That column is an artifact of a run whose weights were never saved (Section S-I-G), and it records 0/150 for M2 and 5/150 for M4, against 0.051 ± 0.103 and 0.100 ± 0.026 here. The archived M4 figure sits below every seed measured, which is consistent with what Section S-I-J already reports for the same condition — the border-mass audit, retraining the same baseline, returned 9/150 rather than the archived 5/150. Nothing in this paper's argument depends on the exact baseline value, which is why the discrepancy is recorded rather than reconciled: what the argument needs is that the baseline is near zero and the corrected pipeline is not, and every measurement of both agrees on that.
...
Section VI-A runs the audit on the case-study pool. This section collects everything the study measured with it elsewhere: seven archives across four application areas, scored from public file listings alone, and then the second pharmaceutical dataset in the detail that establishes the audit's false-negative mode. The two are reported in that order because the first tests whether the audit discriminates at all and the second tests what it misses.
...
**What the split comparison concludes does not depend on the threshold.** Countable Split A leakage runs from 2 clusters under exact matching to 18 at threshold 12 — 0.4% to 3.8% of clusters. Every value in that range is small beside the acquisition confound, so no admissible choice would have changed Section VIII-B. The same holds for Section S-I-V, whose 30 mates are the pairs at distance ≤ 8; a stricter threshold would shrink that set and a laxer one would contaminate it with different products.
