...
"Table 5" and "Fig. 1" are the manuscript's, as is "Section IV-A", while
...
M2 carries an architectural observation that is worth separating from the confound results. It scores 0.865 on the leakage-free split — no statistically significant difference was detected between it and any other model in the roster on the shared internal test partition (Table S5) — and it has the highest external specificity point estimate after correction (0.860, Table 5), with the only negative generalization gap anywhere in this study. It reaches that with 23,938 parameters and no pretraining.
...
> **FIGURE S13.** `paper/figures/fig08_external_generalisation.pdf` — In-distribution authentic accuracy against external specificity before and after three-way normalization, per model, for the run of record. It plots Table 5 of the main paper; M1 bypasses the normalized pipeline by design, so its two Split C bars are the same 0.0% measurement shown twice.

Fig. S13 plots the run of record behind those means.

**The archived pre-normalization run, and why it is superseded rather than co-equal.** Earlier versions of this paper reported the baseline column of the main paper's Table 5 from an archived production run made immediately before three-way normalization became the default, which returned 0/150 for M1 and M2, 104/150 for M3 and 5/150 for M4. Those figures are not lost and never were: the run's own log, with its full per-epoch training curves, is committed at `modeling/results/split_c_eval_log.txt` and has been present since the repository's first commit and in every archived release. `paper/scripts/external_intervals.py` now parses the counts from that log rather than carrying them as constants, so they are traceable rather than transcribed.
...
Section IV-A runs the audit on the case-study pool and summarizes what follows here in its Table 3. This section is the full record behind that summary: seven archives across four application areas, scored from public file listings alone, and then the second pharmaceutical dataset in the detail that establishes the audit's false-negative mode. Every number in the main paper's Table 3 is taken from Table S18 or Table S20 below, and the caveats on both apply to it unchanged. The two are reported in that order because the first tests whether the audit discriminates at all and the second tests what it misses.
...
python modeling/region_substitution.py             # third: region substitution, Table 7
...
python modeling/eval_balanced_external.py          # Table 4; runs once
...
representatives entering Table 4 are a subset. The removals are judgements about what each frame shows, made by one
...
**TABLE S28.** External counterfeit recall on the full 150-image regulatory source, in its native encodings, under five settings of the eligibility screen, with 95% case-clustered bootstrap intervals resampling whole regulatory cases. The predictions are identical across rows; only the subset read changes. S0 is the setting the 46 representatives of Table 4 are drawn from; Section S-IX-D relates this one-sided measurement to Table 4's two-sided one. S3 restores the frames carrying burned-in falsification captions and S4 restores everything, including the WHO logo and the laboratory plates, which is the no-screen bound rather than a defensible set.
...
intervals (Table S28's S0 row); the 46 re-encoded representatives in Table 4
...
Table 4 is the counterfeit-side evidence of record, because it is the only one
...
persisted Split B checkpoints — the same checkpoints that produced Tables 5 to
7, loaded rather than retrained. Nothing about any model or its pipeline was
changed after Table 4 was read. Had anything been, this appendix would say so
and Table 4 would move to the exploratory tier.
