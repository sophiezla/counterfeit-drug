...
"Table 7" and "Fig. 1" are the manuscript's, as is "Section VI-A", while
...
**TABLE S2.**Hyperparameters, in the form the code sets them. Table 3 of the main paper is the reader-facing summary of the same protocol; this table gives the same settings in the names the code uses, together with the learning-rate search range behind them.
...
**M4 is the only model whose calibration survives both capture shifts** (0.061 → 0.098 → 0.055), which is consistent with its accuracy behavior in Table 9 and, like that result, says nothing about whether it is reading packaging — Section S-I-J applies unchanged.
...
> **FIGURE S13.** `paper/figures/fig08_external_generalisation.pdf` — In-distribution authentic accuracy against external accuracy before and after three-way normalization, per model, for the run of record. It plots Table 7 of the main paper; M1 bypasses the normalized pipeline by design, so its two Split C bars are the same 0.0% measurement shown twice.

Fig. S13 plots the run of record behind those means.

**The archived pre-normalization run, and why it is superseded rather than co-equal.** Earlier versions of this paper reported the baseline column of the main paper's Table 7 from an archived production run made immediately before three-way normalization became the default, which returned 0/150 for M1 and M2, 104/150 for M3 and 5/150 for M4. Those figures are not lost and never were: the run's own log, with its full per-epoch training curves, is committed at `modeling/results/split_c_eval_log.txt` and has been present since the repository's first commit and in every archived release. `paper/scripts/external_intervals.py` now parses the counts from that log rather than carrying them as constants, so they are traceable rather than transcribed.
...
Section VI-A runs the audit on the case-study pool and summarizes what follows here in its Table 6. This section is the full record behind that summary: seven archives across four application areas, scored from public file listings alone, and then the second pharmaceutical dataset in the detail that establishes the audit's false-negative mode. Every number in the main paper's Table 6 is taken from Table S18 or Table S20 below, and the caveats on both apply to it unchanged. The two are reported in that order because the first tests whether the audit discriminates at all and the second tests what it misses.

The capture statistics behind Table 4 of the main paper are plotted here rather than there, for space.
...
python scripts/18_capture_method_stats.py          # per-image capture statistics (Table 4, Fig. S14)
...
python modeling/region_substitution.py             # third: region substitution, Table 10
