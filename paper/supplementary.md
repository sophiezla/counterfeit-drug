...
The shape of Table S7 is worth stating plainly against Section VI-C. In-distribution, these models make few errors and most of those errors are explicable. Externally, two of them were wrong on every single image. Both statements are true simultaneously, and a study reporting only the first would be describing a system that does not work as one that works well.
...
Section VI-F of the main paper reports this result and its two readings; the tabulation is here for space. Produced by `modeling/experiment_train_only_operator.py`.
