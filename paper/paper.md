...
The audit is used in Section VI-A and motivated in Section I-A; because it is
what this paper offers for adoption, we state it here as a procedure rather
than leaving it to be reconstructed from the experiments. It needs no external
data, no annotation, no training run and no pixel decoding, and on this dataset
it runs from a file listing in seconds.

**Inputs.** A labelled image dataset; a partition of it that is free of
whatever leakage the study already controls for; and the classifier family the
study intends to use.

1. **Enumerate the acquisition variables the files carry.** Take every
   attribute obtainable without decoding an image: container format, encoded
   size in bytes, stored pixel dimensions, and the aspect ratio they imply.
   These are properties of how the file was produced, not of the object
   photographed. Filename patterns and embedded timestamps belong here too
   where they exist, and are reported separately because they are trivially
   removable and their absence proves nothing.
2. **Partition exactly as the study will.** Fit inside the study's own
   leakage-free split, so the audit inherits the same grouping the models do.
   Auditing under a different partition measures a different quantity.
3. **Fit the intended classifier to those variables alone.** One fit per
   variable, then one on all of them jointly. Report held-out accuracy with an
   interval, using balanced accuracy where the classes are imbalanced. Chance
   is the reference, not zero.
4. **Read container format on its own.** Storage format is never a property of
   the photographed object, so a high score on format is close to decisive,
   whereas encoded size, resolution and aspect ratio mix acquisition with
   content and can be high for reasons that are not confounds. Section VI-A's
   negative control separates exactly along this line.
5. **Report pixel-derived proxies separately if at all.** Statistics such as
   mean brightness require decoding and are neither header fields nor
   independent of content; they belong in their own row, as in Table 5.
6. **Interpret against what the score licenses, not against a threshold.** The
   result establishes the *availability* of a provenance shortcut. It does not
   establish that any model took it, and we publish no threshold, because five
   scorable datasets cannot support a false-positive rate.

**Outputs, and what each supports.** A score at chance says these features
found no acquisition asymmetry — not that the dataset is clean, since a
publisher who normalized the archive before release destroys the traces
without touching the confound (Section VI-A, the Roboflow row). A middling
score on the ambiguous axes is a reason to investigate, and may reflect a
content difference rather than an acquisition one. A high score on format means
acquisition determines the label, and from there no in-distribution accuracy on
that dataset can distinguish the intended task from provenance recognition.

**Two companion checks the audit does not replace**, because on our own data
each caught something it missed: a content pass over a sample of each class
(Section S-I-A), which is what identified the modality confound of Section
III-B; and cross-source near-duplicate hashing (Section IV-B), which is what
established that two sources treated as independent were not. Section VII-F
gives the defect taxonomy these three checks jointly cover.
...
Occlusion asks a local question — how far does hiding *this patch* move the score — and a decision distributed over a large region answers it weakly wherever that region is. So the M4 result is uninformative rather than negative, and a global test settles it.

**Region substitution identifies the region, and it is the surround for both backbones.** Each external image is put through the production pipeline and one region of the resulting input is overwritten before the forward pass: a centre square covering half the frame, or its complement. The two regions are area-matched to within half a point, which is what makes the comparison readable — substituting a region is itself a distribution shift, so only the asymmetry between the two directions is interpretable. Two fills are used because they fail differently: the ImageNet channel mean, a flat region, and Gaussian noise, a high-entropy one. The intact condition reproduces each model's specificity of record before anything is reported (Table 10).

**TABLE 10.** Region substitution on both external sets. Each cell is authentic-class specificity after overwriting one region of the model's input; the intact row is the unmodified production pipeline and reproduces the value of record exactly. "Outer" is everything outside a centre square covering half the frame and "inner" is the square itself, so the two are area-matched (0.5025 against 0.4975) and the asymmetry between them is not an artifact of how much was destroyed. The last two rows use the 0.642 border ring and 0.161 centre box of the occlusion analysis, which are *not* area-matched and are given for continuity with it. Both external sets are authentic-only, so every figure is a specificity. Parenthesized values are changes from intact, in percentage points.
...
The substantive point no longer rests on the weaker method. **The models corrected by Eq. (8) have not stopped depending on acquisition; both now depend on a different acquisition-linked cue — the photographic surround — which the audit of Section VI-A did not test and which both external sets hold constant.** That is the finding Section VII-E develops.
...
Table 12 separates what this study demonstrates from what it hypothesizes or leaves unmeasured; the paragraphs after it state each limitation in turn, and Section S-II gives the evidence for and against each. The list is complete but compressed.

**TABLE 12.** Evidence status of the claims in this paper, on the three levels set out in Section I-A. "Demonstrated" means measured here on the data named; "supported" means consistent with evidence reported here or elsewhere but not measured as a claim in its own right; "not established" means the paper argues for it without measuring it; "not measured" means the quantity was never estimated at all.
...
| External acquisition shift exposes the failure | Demonstrated for authentic-class specificity: 97.4% in-distribution against 6.0% externally | Table 7 |
| Normalization removes much of the acquisition signal | Demonstrated for M2 and M4; not demonstrated for M3 | Tables 7 and 8 |
| The corrected models recognize packaging | Demonstrated against: overwriting the centre of the frame costs neither backbone anything | Table 10 |
| The corrected models depend on the photographic surround | Demonstrated on both external sets: overwriting the outer half costs 22–50 points | Table 10 |
| Correcting one provenance signal can expose another | Demonstrated on this dataset: the corrected models' recovered specificity localizes to the photographic surround, an acquisition property the audit does not test | Table 10, Section VII-E |
...
**Repository.** All code and derived artifacts are at `https://github.com/sophiezla/counterfeit-drug`, archived at the concept DOI **10.5281/zenodo.21936720**, which resolves to the most recent release. The release accompanying this manuscript is **v1.2.0**, archived at doi:10.5281/zenodo.22223271, and it is the exact state of the code that produced every number reported here; `README.md` and `CITATION.cff` name it, and the earlier releases they list are marked superseded. It holds the data pipeline, the four model implementations, every analysis and figure script, the per-image statistics and split assignments, the persisted checkpoints, and the sources of this manuscript and its supplement.
