...
**Repository.** All code and derived artifacts described below are available at `https://github.com/sophiezla/counterfeit-drug` and archived at **doi:10.5281/zenodo.21936721** (release v1.0.0, 14 August 2026), which is the exact state of the code that produced every number reported here. The version-independent identifier doi:10.5281/zenodo.21936720 resolves to the most recent release. The repository contains the data pipeline, the four model implementations, every analysis and figure-generation script, the per-image statistics and split assignments, the persisted model checkpoints, and the manuscript source from which this document is built.
