...
5. **A label-free correction, ablated per axis, per architecture, per constant and per composition order**, together with a complete attention audit of what the surviving accuracy rests on (Sections VI-G, VII). Varying the *order* of the three operators — which a preprocessing description normally leaves implicit — matters more than any of their magnitudes: it moves external accuracy across a 50-point range under a 2.7-point in-distribution range, and the ordering scoring highest in-distribution scores lowest externally (Section VII-B, Table 10).
...
An authentic-only set measures the false-positive rate on genuine packaging and says nothing about counterfeit recall. Section V-F describes the synthetic proxy built to probe that direction, and Section IX states the limitation that remains.
...
**Brightness placement is a second-order effect.** Within the three sound orderings, moving the brightness rescale changes external accuracy by 6 points (0.820 → 0.880), against the 28-point gap between the groups. This is the distinction of Section S-I-P in another guise: the two bottleneck operators are load-bearing and position-sensitive, the location-shifting operator is neither.
...
The datasets audited here failed in different ways, and a third near-failure was caught while building our own evaluation set. "Provenance confound" is not one defect but a small family, each member needing a different check. Section S-I-R gives the full treatment with instances and costs; the members are:
...
**Type E, a real content difference the audit reads as a confound.** This is the audit's false-positive mode. *Detected by* comparing the audit's axes against each other: storage format is never a property of the photographed object, whereas size, resolution and aspect ratio mix acquisition with content. *Instance:* the signature corpus of Section VI-E, format 0.500 and size 0.843, where the size signal is most likely ink coverage.
...
**Both external sets share one backdrop.** They vary device and lighting on the same products, so the surround cue that Section VI-G identifies is untested. This is the most consequential gap in the study, and it qualifies every claim that the correction "holds" across a capture shift.

**"Leakage-free" means product-identity leakage only.** No partition of this pool can decorrelate acquisition, because the counterfeit class exists in exactly one capture pipeline. Cross-validating across sources — the strongest available answer — is impossible here for the same reason: the only other public authentic/counterfeit pharmaceutical dataset has a counterfeit class that is unusable at any position in a fold, so a source-held-out fold would contain no negatives.

**The correction is a preprocessing bottleneck, not a domain-adaptation method, and is not compared against one.** Eq. (8) should be read as a *zero-target-sample baseline*: the performance recoverable when nothing is known about the deployment distribution. Anyone holding even unlabeled target images is in a stronger position and should benchmark second-order alignment, distribution matching or an adversarial confusion head against it. We expect those to win; the margin is unmeasured here, and the reason for the omission is that every such method consumes target data, which would forfeit Split C as an external evaluation.
...
**The generality claim is a hypothesis with a stated test, not a measured rate.** Two datasets in one application area, seven audited across four, and one convergent report from an unrelated field [30] motivate the mechanism; they do not establish how often it occurs. Section IX names the survey that would.
