...
**Repository.** [To be completed before submission: the code and derived artifacts described below must be deposited in a public, citable archive — a GitHub repository with a Zenodo DOI, or equivalent — and the DOI given here. This paper's contribution is in part a reproducibility argument, and none of the verification described in this section can be performed by a reader until that link exists. Nothing in the repository requires the image archives themselves: the five analysis scripts named below read committed per-image statistics and split assignments only.]
