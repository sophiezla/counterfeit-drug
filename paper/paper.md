...
**ABSTRACT** In image datasets asking whether an object is genuine, the inauthentic class is scarcer and often obtained differently: screen-captured, scraped, or generated. The label then predicts acquisition rather than the property of interest, and every held-out partition inherits it alike. We call this **class-conditional provenance confounding** and propose a pre-training **provenance audit**: before evaluating an image classifier, fit it to acquisition metadata alone, under your own leakage-free split, and report the number. In a public counterfeit-medicine dataset three header fields reproduce the labels exactly, no pixel decoded: every counterfeit file is a PNG screen capture and every authentic one a JPEG photograph. No in-distribution result on that pool then separates packaging from provenance. Correcting the split changes accuracy by at most 6.8 points across four model families, and varying leakage alone on a fixed test set by 0.3; on 150 independently captured authentic photographs the strongest model's authentic-class specificity falls from 97.4% to 6.0%. Label-free normalization of resolution, brightness and compression raises it to 86% and 81% for two of three, the third inside seed variance. We offer it as a diagnostic, not a remedy. Eliminating one signal can expose another: a second capture drops the best corrected model to 63%, operator order alone moves external specificity 50 points while ranking the worst pipeline best in-distribution, and overwriting the outer half of the frame costs both backbones 22 to 50 points where an area-matched centre costs nothing. Removing a confound is itself dataset construction. We report where the audit misfires.
...
What separates this paper from [31] is level rather than subject. That work diagnoses confounding one dataset at a time, with methods needing the images and a trained classifier; we ask what makes such datasets predictable in advance from how a class was obtained, reduce the diagnosis to a screen that reads file listings before any model exists, and follow the correction through to show that removing a confound is itself dataset construction (Sections VI-G, VII-E). The closest published analogue outside medicine is [30], described in Section I-A.
...
4. **Read container format on its own.** Storage format is never a property of the photographed object, so a high score on it is close to decisive, whereas encoded size, resolution and aspect ratio mix acquisition with content; Section VI-B's negative control separates along exactly this line.
...
|---|---|---|
| **Confirmatory** (pre-specified) | The provenance audit; the Split A vs. Split B comparison and the paired leakage experiment; the baseline external evaluation of the four models on Split C | Sections VI-A–VI-C, and the baseline column of Table 4 |
| **Exploratory** (post-hoc) | The normalization itself; the selection of its three axes; the choice of its constants; the operator ordering; the resolution sweep; the attribution investigations; Split D | Sections VI-D–VI-G |

The second tier is where the strongest-looking numbers in this paper live, and it is also where Split C stops being an untouched external set: the three normalization axes were nominated *because* Table 1 showed those statistics separating the training pool from Split C, which makes Split C target-informed development data for that experiment and for nothing else (Sections V-D, VI-D, VIII). The 86% and 81% figures are therefore not clean external-validation results, and we do not report them as such. Section S-I-S re-derives the same three axes from the training partition alone, under a threshold declared in advance, which is the closest this study comes to repairing that.

### A. The capture-method confound in the case-study dataset
...
The consequence needs no attribution. Because provenance predicts the label perfectly here, in-distribution accuracy on this pool cannot distinguish a classifier that learned packaging semantics from one that learned provenance-associated cues, and is uninformative about the stated task — ours as much as anyone's. What a particular model actually used is a separate question, and Section VI-F is our attempt at it. Third, **the pixel-derived proxy is the weakest candidate here, not the strongest.** Brightness alone reaches 0.716 on Split B, below resolution (0.946) and file size (1.000), despite having the largest *t*-statistic: the brightness distributions differ in mean but overlap substantially, whereas the file-size distributions barely overlap. Rank candidate confounds by fitting a classifier to each, not by *t*-statistic — it costs no more, and it is why the audit starts at the header.

### B. The audit on datasets other than the case study

A screen that fires on every dataset carries no information, so the same procedure was run on six further public archives across four application areas (Table 3; Section S-I-W gives the full record). Four of the seven were scorable, the case study among them, the other three reaching only one class before the listing endpoint's file limit; the Roboflow archive of Section III-B, audited separately in Table S20, is the fifth row of Table 3. The five separate into the three outcomes the method admits.
...
Two conventions govern the table. Feature sets differ by row, because the two source audits had different inputs available: the four listing-audited rows supply format and encoded size, the Roboflow row adds resolution and aspect ratio, and the Kaggle row returns 1.000 under both. And a dataset admitting no score is not one that scored low — the three archives absent here reached only one class before the listing endpoint's file limit, a property of the sampling method (Section S-I-W).
...
Taken together the three readings fix what the audit is. It establishes that a provenance shortcut is *available*; it certifies nothing, because a publisher can erase the traces it reads without touching the confound underneath, and a separation it does find may belong to the objects rather than to how they were acquired. **A provenance audit is a diagnostic, not a clearance** — a high score on format is close to decisive, and a low one is where the investigation starts rather than where it ends.

### C. In-distribution performance, and the size of the leakage effect
...
Three conventions govern the table. The gap column is in-distribution minus external for the normalized model, *not* the effect of normalization, which is the difference between the two preceding columns. The Wilson intervals quantify sampling uncertainty on a fixed trained model, not training-run variance, which Table 5 measures separately. And one earlier measurement is superseded rather than reported: an archived run predating three-way normalization returned 104/150 for M3 and 5/150 for M4, from a pipeline carrying the two defects of Section S-I-G — a hard-coded learning rate for M2 and unseeded augmented feature passes for M3 and M4, both since fixed. Section S-I-U diagnoses that divergence per model and supersedes the run; it is not quoted as a result anywhere in this paper.
...
Repeating both conditions at five seeds settles which movement is a finding and which is initialization (Table 5). M2 and M4 separate by 86 and 76 points against standard deviations of three to ten, the worst normalized seed far above the best baseline seed in both; M3 moves 0.715 ± 0.057 to 0.724 ± 0.047, the two sets of five interleaved. **We therefore claim no normalization benefit for M3**, as a measurement rather than a caution, and rest the headline claim on M2 and M4. Only separations of this size are read as findings anywhere in this paper: five seeds bound seed sensitivity rather than estimating it, so what makes a reading safe here is that the separation is far larger than the quantity being estimated, and a point or two between models, orderings or seeds is inside what 74 to 150 images and five runs can resolve.

**TABLE 5.** Five-seed sensitivity analysis, seeds 42–46, mean ± sample standard deviation. Every other number in this paper comes from one run at seed 42; this table says how much of it is initialization. Split assignments, preprocessing and learning rates are fixed across seeds, so the training seed is the only thing that moves. Both conditions come from the same current harness as Table 4, whose two columns are this table's seed-42 draw. Splits C and D are authentic-only, so those columns are specificities. M1 is omitted: a convex fit with a deterministic solver and no augmentation, its accuracy is identical under every seed. Per-seed values are in Section S-I-U.
...
### E. A second capture shift, and what it costs the headline
...
**TABLE 6.** The same corrected models on two external distributions. Both external sets hold authentic images only, so the reported quantity is authentic-class specificity — the fraction correctly classified as authentic — with 95% Wilson intervals, and none of it estimates counterfeit recall. The two sets cover the same packages under different capture conditions but cannot be aligned image by image (Section III-F), so the columns are compared as independent proportions.
...
Two of the three narrow claims made elsewhere, and we state those first.
...
**The Grad-CAM categorization is corroborating evidence only.** It covers the 62 maps this study produced, each placed in one of two categories by a single annotator, and Section VIII says why that is not a reproducible measurement. It agrees with the occlusion analysis on three of the four groups above and disagrees on the fourth, reading M4's correct external answers as surround-driven where occlusion cannot distinguish them from uniform. Where the two disagree we report the occlusion result and record the disagreement.
...
Two consequences follow. Both external sets hold the backdrop fixed, so neither disturbs the cue identified here, and the backbones' stability across that shift is what a surround-dependent rule predicts. And what the normalized models see in aggregate stays an open question, a 128 px bottleneck degrading activation-based attribution as much as it degrades the input (Section S-I-J). **The models corrected by Eq. (8) have not stopped depending on acquisition; both now depend on a different acquisition-linked cue — the photographic surround — which the audit of Section VI-A does not test and which both external sets hold constant.** That is the finding Section VII-E develops.
...
Table 8 separates what this study demonstrates from what it hypothesizes or leaves unmeasured. The paragraphs after it state each limitation in turn, and Section S-II gives the evidence for and against each.
...
| In-distribution validation does not surface the confound | Demonstrated on this dataset: stratification, grouped cross-validation, bootstrap intervals and near-duplicate grouping all pass | Sections VI-A, VI-C |
| Near-duplicate leakage is the smaller effect here | Demonstrated: +0.3 points [−1.9, +2.4] on a fixed test set | Section VI-C |
...
| The corrected models recognize packaging | Not established: a centre substitution costing neither backbone anything does not demonstrate packaging recognition | Table 7 |
...
| The corrected models generalize across acquisition | Not established: both external sets come from one archive and share one backdrop, so only a device and lighting shift was tested | Sections III-F, VI-E |
...
**No public counterfeit-labeled set was available to run that test**, and the reason is a property of what this area publishes rather than an experiment we neglected. Of the three archives inventoried (Section III-A), the one verified independent of the modeling pool carries no counterfeit label and became Split C. The one that does carry counterfeit labels is not external to that pool at all — 42.3% of the Kaggle pool has a near-duplicate in it (Section III-C); its counterfeit class is 57/57 advisory graphics with the ground-truth word rendered inside the image, leaving two usable counterfeit photographs (Section III-B); it documents no collection protocol or per-image provenance; and Section VI-B finds its acquisition traces already erased by publisher-side resizing and re-encoding, so it cannot be checked for acquisition balance either. Its labels are not a substitute for an external counterfeit test.

**Both external sets share one backdrop.** They vary device and lighting on the same products, so the surround cue Section VI-F identifies is untested — the study's most consequential gap, and it qualifies every claim that the correction "holds" across a capture shift.
...
**Acquire an external set that varies the photographic setting, and one that is counterfeit-labeled.** These are the two evaluations this study most needs and could not build. Photographs of the same packages against varied surfaces would test the surround cue of Section VI-F directly, at a fraction of the cost of either. The counterfeit-labeled set would change most; its requirements are specific — independently photographed, verified by the pHash procedure of Section IV-B, and with acquisition balanced across classes so it does not import the confound it exists to test.
...
Two qualifications come with it. The audit is necessary but not sufficient: on a second pharmaceutical dataset it returned 0.717 while that dataset was confounded at least as badly, its publisher having normalized the acquisition traces away without removing the confound. And it says nothing about whether a model generalizes; only data someone else acquired does that. That the same signature is documented independently in generated-image detection [30], and appears in a third archive in our own pilot audit, is what a structural cause predicts — a mechanism we would expect wherever a scarce class must be manufactured or harvested separately from an abundant one, and not a rate we have measured.
