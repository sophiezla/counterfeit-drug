...
Two consequences follow. Split C and Split D are the same products on one dark backdrop, so neither disturbs the cue identified here: both backbones holding up across that shift is what a surround-dependent rule predicts, not evidence of robustness. And we make no claim about what the normalized models see in aggregate, because a 128 px bottleneck degrades activation-based attribution as much as it degrades the input (Section S-I-J). {\bfseries The models corrected by \eqref{eq:8} have not stopped depending on acquisition; both now depend on a different acquisition-linked cue --- the photographic surround --- which the audit of Section VI-A does not test and which both external sets hold constant.} That is the finding Section VII-E develops.
...
None of this makes the operator useless; it fixes what it is for. One objection has to be met first: the axes were chosen because Table~\ref{tab:1} showed them separating the pool from Split C, so intervening on them might seem guaranteed to help. It is not --- a bottleneck that removed the discrepancy while destroying the signal would have driven in-distribution accuracy to chance, as it does for M1 (Table S13). What the experiment supports is therefore an {\itshape interventional} claim about three named statistics and nothing wider: removing the discrepancy on resolution, brightness and compression recovers 85.9 points of external specificity for M2 and 76.0 for M4, against baseline specificities of 0.051 and 0.100, and recovers nothing outside seed variance for M3. What that leaves is a residual gap, not a repair: 20.1 points for M3 and 16.7 for M4 at seed 42 (Table~\ref{tab:4}), and for M2 a negative one. Two things it does not establish should be said in the same breath. It does not identify {\itshape which} of the three, or what within them, the models were responding to, since the axes are correlated and were removed together. And it is not evidence that the corrected models learned to recognize packaging --- Section VI-F indicates the opposite for both backbones.
...
Table~\ref{tab:8} separates what this study demonstrates from what it hypothesizes or leaves unmeasured; the paragraphs after it state each limitation in turn, and Section S-II gives the evidence for and against each. The list is complete but compressed.
...
The corrected models recognize packaging & Demonstrated against: overwriting the centre of the frame costs neither backbone anything & Table~\ref{tab:7} \\
...
Two qualifications keep that recommendation honest. The audit is necessary but not sufficient: on a second pharmaceutical dataset it returned 0.717 while that dataset was confounded at least as badly, its publisher having normalized the acquisition traces away without removing the confound. And it says nothing about whether a model generalizes; only data someone else acquired does that. That the same signature is documented independently in generated-image detection \cite{ref30}, and appears in a third archive in our own pilot audit, is what a structural cause predicts --- a mechanism we would expect wherever a scarce class must be manufactured or harvested separately from an abundant one, and not a rate we have measured.
