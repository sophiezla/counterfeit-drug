...
{\bfseries Type E, a real content difference the audit reads as a confound.} This is the audit's false-positive mode. {\itshape Detected by} comparing the audit's axes against each other: storage format is never a property of the photographed object, whereas size, resolution and aspect ratio mix acquisition with content. {\itshape Instance:} the signature corpus of Section VI-C, format 0.500 and size 0.843, where the size signal is most likely ink coverage.
