...
--- the label a deterministic function of acquisition. Here container format alone leaves no residual entropy: every counterfeit-labeled file is a PNG and every authentic-labeled file a JPEG, across all 510 images. A {\bfseries provenance audit} measures how close a dataset sits to that extreme by fitting a classifier to $A$ alone and reporting its held-out accuracy --- up to sampling error, a lower bound on how well acquisition can predict the label.
...
We separate three levels of evidence here and hold to them throughout. {\bfseries Demonstrated}: the case-study dataset is completely provenance-confounded, container format reproducing the label on all 510 images (Section VI-A). {\bfseries Supported}: the mechanism is consistent with an independent report in another application area. In generated-image detection, real images are harvested as lossy JPEGs at modest resolution while generated images are written as lossless PNGs at native size, and Grommelt {\itshape et al.} \cite{ref30} show that on GenImage this makes format, compression and size predictive of the label, that detectors partly become JPEG detectors, and that equalizing those factors shifts cross-generator performance by more than 11 points --- no shared data, application area or method of discovery, and the same confound, which is what a structural cause predicts and a coincidence does not. {\bfseries Not established}: how frequently this occurs. A prevalence estimate would require auditing a representative sample of such datasets, which neither this paper nor, as far as we can determine, any other has done; Section VI-B's pilot audit of seven archives finds both a totally confounded case and a clean one, enough to show the audit discriminates and far short of a rate. Table~\ref{tab:8} records every claim in this paper against these three levels, and Section VII-G says what would falsify the mechanism.
...
The result was not the one the design anticipated. Correcting the split changed in-distribution accuracy by at most 6.8 points; external validation changed the picture. On 150 authentic photographs from an independent source, two of four models classified {\itshape zero} correctly and the strongest in-distribution model almost none, despite 97.4\% on the authentic class of its own test partition. Looking for an explanation led to the dataset itself: its two classes were not merely photographed differently but {\itshape acquired} differently, every counterfeit-labeled file being a screen capture (\texttt{Screenshot*.png}) and every authentic-labeled file a downloaded photograph (\texttt{images*.jpg}), correlating with the label exactly 1.0 across the pool. Any model, including a 97-parameter linear classifier on a color histogram, has unobstructed access to it. It is the hidden stratification of \cite{ref7}, total rather than partial (Section II-B).
...
Three public sources were inventoried, all considered for the modeling pool: the Kaggle {\itshape Fake vs Real Medicine} set \cite{ref19}, 661 unique files re-listed across a bundled split, licence stated as "Unknown"; Roboflow {\itshape Counterfeit\_med\_detection} v4 \cite{ref21}, 4,260 files under CC BY 4.0, including the publisher's own 3$\times$ rotation and exposure augmentation; and the Mendeley {\itshape Mobile-Captured Pharmaceutical Medication Packages} archive \cite{ref20}, 3,900 files across six devices under CC BY 4.0. Two were excluded from modeling for the reasons below and the third became the external evaluation set; Table S1 gives the full inventory.
...
Three kinds of uncertainty are kept apart throughout, because several readings turn on which is being quoted. {\bfseries Sampling uncertainty} is what a finite evaluation set supports for one trained model, and is what every bracketed interval describes unless a table says otherwise. {\bfseries Training-run variability} is what a different initialization would have produced, measured separately across five seeds (Table~\ref{tab:5}). Neither bounds the other, and the difference is not academic: Section VI-D reports an effect clearly outside sampling uncertainty and entirely inside seed variance. {\bfseries Distribution-shift uncertainty} is not addressed by either; only evaluation on data we did not collect speaks to it, which is why Splits C and D exist.
...
Both transfer models are therefore {\bfseries linear probes on a fixed representation}, which is what this study's question calls for rather than a concession to it: the question is what a change in the {\itshape input distribution} does, and a probe holds the representation constant while the input changes, so any movement is attributable to the input, whereas a fine-tuned network confounds the two by re-adapting its features. Freezing also keeps M3 and M4 balanced --- both train exactly one linear layer --- and the compute budget within central-processing-unit (CPU) only hardware. What a probe cannot say is what fine-tuning would do, in either direction (Section IX).
...
\item {\bfseries Read container format on its own.} Storage format is never a property of the photographed object, so a high score on it is close to decisive, whereas encoded size, resolution and aspect ratio mix acquisition with content; Section VI-B's negative control separates along exactly this line.
...
{\bfseries Two companion checks the audit does not replace}, each having caught something it missed on our own data: a content pass over a sample of each class (Section S-I-A), which identified the modality confound of Section III-B, and cross-source near-duplicate hashing (Section IV-B), which established that two sources treated as independent were not. Section VII-F gives the taxonomy these three checks jointly cover.
...
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.15}
\begin{tabularx}{\columnwidth}{>{\hsize=0.331\hsize\raggedright\arraybackslash\hyphenpenalty=10000\exhyphenpenalty=10000}X>{\hsize=2.031\hsize\raggedright\arraybackslash\hyphenpenalty=10000\exhyphenpenalty=10000}X>{\hsize=0.638\hsize\raggedright\arraybackslash\hyphenpenalty=10000\exhyphenpenalty=10000}X}
\toprule
{\bfseries Tier} & {\bfseries What it covers} & {\bfseries Where} \\
\midrule
{\bfseries Confirmatory} (pre-specified) & The provenance audit; the Split A vs. Split B comparison and the paired leakage experiment; the baseline external evaluation of the four models on Split C & Sections VI-A--VI-C, and the baseline column of Table~\ref{tab:4} \\
{\bfseries Exploratory} (post-hoc) & The normalization itself; the selection of its three axes; the choice of its constants; the operator ordering; the resolution sweep; the attribution investigations; Split D & Sections VI-D--VI-G \\
\bottomrule
\end{tabularx}
\end{center}


The second tier is where the strongest-looking numbers in this paper live, and it is also where Split C stops being an untouched external set: the three normalization axes were nominated {\itshape because} Table~\ref{tab:1} showed those statistics separating the training pool from Split C, which makes Split C target-informed development data for that experiment and for nothing else (Sections V-D, VI-D, VIII). The 86\% and 81\% figures are therefore not clean external-validation results, and we do not report them as such. Section S-I-S re-derives the same three axes from the training partition alone, under a threshold declared in advance, which is the closest this study comes to repairing that.

\subsection{The capture-method confound in the case-study dataset}

The strongest result in this study needs four learned parameters and no decoded pixels: a logistic regression on three header fields separates the two classes perfectly on the leakage-free partition (Table~\ref{tab:2}). This subsection builds to it.

Every one of the 510 pool filenames falls into exactly one of two patterns, and {\bfseries the pattern predicts the class label with no exceptions}: 272/272 authentic files are \texttt{images*.jpg} and 238/238 counterfeit files \texttt{Screenshot*.png}. This is not an artifact of our filtering --- it holds in the archive as distributed, where all 240 files in \texttt{Fake/} are \texttt{Screenshot*.png} and all 421 in \texttt{Real/} are \texttt{images*.jpg} --- so any study using this dataset inherits it in full, and a classifier reading nothing but the file extension achieves {\bfseries 100\% accuracy}. Table~\ref{tab:1} gives what the two pipelines look like in the file, and where the external set sits beside them.
...
The confound is visible in the simplest model's decision function. Of M1's 96 coefficients, 93 lie within $\pm$0.35 of zero while the top intensity bin (248--255) of each channel carries a large negative weight ($\beta$ = $-$2.86, $-$2.84, $-$2.95) --- "many near-white pixels $\rightarrow$ authentic" --- and the logit decomposition of \eqref{eq:10} puts those three bins in the top three positions by mean |$\varphi$|, at 0.079--0.082 against 0.002 for the fourth (Section S-I-P). M1's 83.8\% accuracy therefore appears driven largely by near-white pixel prevalence rather than by packaging.
...
{\bfseries It discriminates.} Two archives return 1.000 on format and two at or near chance. The generated-image pair is the sharper case: two datasets built for the same task by different people return 1.000 and 0.577, which is also a caution against reading the mechanism as a law.

{\bfseries It has a false-negative mode, and publisher-side tidying causes it.} The Roboflow archive is confounded at least as severely as the case study --- 57/57 of its counterfeit-labeled source images are advisory graphics carrying the ground-truth word in the pixels (Section III-B) --- yet format returns 0.500 and the full set 0.717, because the publisher resized to 640 $\times$ 640 and re-encoded before release. A tidied dataset is {\itshape harder} to audit than a raw one, so a middling score is a reason to investigate, not a clearance.

{\bfseries Its false-positive mode is on the ambiguous axes.} Genuine and forged signatures in BHSig260 are written on one paper and digitized by one procedure, so format returns exactly 0.500 while encoded size returns 0.843, most likely because forged signatures differ in stroke complexity and so in ink coverage. Encoded size, resolution and aspect ratio mix acquisition with content; container format does not. We publish no threshold: five scorable datasets cannot support a false-positive rate.

Taken together the three readings fix what the audit is. It establishes that a provenance shortcut is {\itshape available}; it certifies nothing, because a publisher can erase the traces it reads without touching the confound underneath, and a separation it does find may belong to the objects rather than to how they were acquired. {\bfseries A provenance audit is a diagnostic, not a clearance} --- a high score on format is close to decisive, and a low one is where the investigation starts rather than where it ends.

\subsection{In-distribution performance, and the size of the leakage effect}

One definition governs what follows. "Leakage-free" here means free of {\bfseries product-identity} leakage: Split B guarantees that no perceptual-hash cluster of near-duplicate photographs straddles a partition or a fold. It makes no claim about acquisition. Because every counterfeit-labeled image was produced by one capture pipeline and every authentic-labeled image by another, {\itshape no} partition of this pool can place a capture process on one side of a fold, and grouped cross-validation inherits the confound at full strength. Split B corrects one of the two problems, not both.
...
Table~\ref{tab:4} gives the central result of this paper, under the convention of Section V-A. Split C holds authentic images only, so every number in it is an {\bfseries external authentic-class specificity}, and what it records is a false-positive collapse under acquisition shift --- not a demonstration that counterfeit {\itshape detection} generalizes poorly, since counterfeit sensitivity, F1 and balanced accuracy under shift are measured nowhere in this study (Section III-E).

\begin{table*}[!t]
\caption{External authentic-class specificity on 150 independently captured authentic photographs; every figure is a true-negative rate and none estimates counterfeit recall. {\bfseries Both conditions come from the current deterministic pipeline at seed 42}, so the two columns differ only by the operator of \eqref{eq:8} and the comparison is within-run; Table~\ref{tab:5} gives both across five seeds. The in-distribution reference is authentic-class accuracy on each model's own Split B test partition (n = 39), and the last column is the in-distribution-minus-external gap of the normalized model, Eq. (S5) of the supplement. Intervals are 95\% Wilson on the counts given as k/n.}
\label{tab:4}
\centering
\footnotesize
\setlength{\tabcolsep}{3pt}
...
Occlusion asks a local question --- how far does hiding {\itshape this patch} move the score --- which a decision spread over a large region answers weakly wherever that region is. The M4 result is therefore uninformative rather than negative, and a global test settles it.

{\bfseries Region substitution identifies the region, and it is the surround for both backbones.} One region of each external image's production-pipeline input is overwritten before the forward pass: a centre square covering half the frame, or its complement. The two are area-matched to within half a point, which is what makes the comparison readable, since substituting a region is itself a distribution shift and only the asymmetry between the two directions is interpretable. Two fills are used because they fail differently: the ImageNet channel mean, flat, and Gaussian noise, high-entropy. The intact condition reproduces each model's specificity of record before anything is reported (Table~\ref{tab:7}).

\begin{table*}[!t]
\caption{Region substitution on both external sets. Each cell is authentic-class specificity after overwriting one region of the model's input; the intact row is the unmodified production pipeline and reproduces the value of record exactly. "Outer" is everything outside a centre square covering half the frame and "inner" is the square itself, so the two are area-matched (0.5025 against 0.4975) and the asymmetry is not an artifact of how much was destroyed. The last two rows instead reuse the occlusion analysis's 0.642 border ring and 0.161 centre box, which are {\itshape not} area-matched. Both sets are authentic-only, so every figure is a specificity; parenthesized values are changes from intact, in points.}
\label{tab:7}
\centering
\footnotesize
\setlength{\tabcolsep}{4pt}
...
{\bfseries The reading we take forward} is that neither corrected backbone is shown to recognize packaging and both depend on the photographic surround, which both external sets hold fixed. We withdraw the earlier reading that the two behave differently on their correct answers.

Two consequences follow. Both external sets hold the backdrop fixed, so neither disturbs the cue identified here, and the backbones' stability across that shift is what a surround-dependent rule predicts. And what the normalized models see in aggregate stays an open question, a 128 px bottleneck degrading activation-based attribution as much as it degrades the input (Section S-I-J). {\bfseries The models corrected by \eqref{eq:8} have not stopped depending on acquisition; both now depend on a different acquisition-linked cue --- the photographic surround --- which the audit of Section VI-A does not test and which both external sets hold constant.} That is the finding Section VII-E develops.

One further conclusion cautions against a common practice. {\bfseries A visually convincing, product-centered Grad-CAM map is not evidence that a model learned the intended task.} That rests on the un-normalized model, whose input is unaltered and whose attribution is not in question: it is strongly product-centered, 0.563 of its mass in a centre box covering 0.161 of the frame, and it classifies almost no external image correctly (Table~\ref{tab:4}). Across the two conditions, spatial concentration of attribution {\itshape anti-correlates} with external specificity.
...
The normalization of \eqref{eq:8} does what it was designed to do: label-free, under 17 ms per image, and worth {\bfseries +85.9 points [+81.7, +89.7] of external authentic-class specificity for M2 and +76.0 [+69.9, +81.5] for M4} across five seeds, paired on the 150 images (Table S23) --- taking them to levels of 86.0\% and 80.7\% at seed 42, and leaving M3's apparent gain inside seed variance. Its in-distribution cost should be quoted with its scope: {\bfseries at seed 42, the run of record, normalization changed Split B accuracy by no more than 1.4 points in either direction; across the five seeds of Table~\ref{tab:5} the changes were $-$3.0 points for M2 and +3.0 for M4.} We nevertheless report the operator as an instrument and not a remedy, and four results already given are the reason.

{\itshape The recovered specificity rests on the photographic surround rather than the packaging} (Section VI-F): overwriting the outer half of the frame costs both backbones 22 to 50 points, while overwriting an area-matched centre region costs neither anything, and both external sets share one dark backdrop. {\itshape The exchange is not stable}: M2, the best corrected model on Split C, falls to 0.463 on a second capture of the same products and to 0.627 $\pm$ 0.135 across five seeds, which a correction that had truly removed the dependence on acquisition would not do. {\itshape The pipeline is under-determined by its own description}: permuting operator order alone moves external specificity 50 points under a 2.7-point in-distribution range with the ranking inverted, so a detail methods sections leave implicit decides most of the result and the usual way of settling it picks the worst option. {\itshape Removing more information keeps helping}: tightening the resolution bottleneck from 128 px to 96 px raises external specificity to 0.873, which is what a coarse distribution match predicts and not what restored reading of printed detail predicts.

None of this makes the operator useless; it fixes what it is for. One objection has to be met first: the axes were chosen because Table~\ref{tab:1} showed them separating the pool from Split C, so intervening on them might seem guaranteed to help. It is not --- a bottleneck that removed the discrepancy while destroying the signal would have driven in-distribution accuracy to chance, as it does for M1 (Table S13). What the experiment supports is an {\itshape interventional} claim about three named statistics and nothing wider, and what it leaves is a residual gap rather than a repair: 20.1 points for M3 and 16.7 for M4 at seed 42 (Table~\ref{tab:4}), and for M2 a negative one. It does not identify {\itshape which} of the three, or what within them, the models were responding to, the axes being correlated and removed together; and it is not evidence that they learned to recognize packaging, Section VI-F locating the recovered specificity on the surround for both backbones.

The conclusion we would defend is accordingly not that a preprocessing solution was developed. It is the narrower and, we think, more useful one: {\bfseries eliminating one provenance signal can expose another, so a correction is itself dataset construction and is owed the same audit as a dataset.} The evidence is measured rather than argued. The audit of Section VI-A named three acquisition statistics and \eqref{eq:8} removed them; region substitution then shows that what the corrected models read in order to call an external photograph authentic lies outside the middle of the frame, for both backbones and on both external sets, and the middle of the frame is where the packaging is (Table~\ref{tab:7}). The surround is an acquisition property --- backdrop, staging and lighting are chosen by whoever took the photograph, not by the manufacturer --- and one the audit does not test, because it leaves no trace in a file header. A practitioner who ran the audit, applied the correction and reported the resulting external number would have reported a real improvement produced by a mechanism they had not audited.

\subsection{A taxonomy of provenance defects, and what detects each}

"Provenance confound" is not one defect but a small family, each needing a different check, and the datasets audited here failed in different ways. {\bfseries Type A} is the acquisition-statistic confound this paper is mostly about (the Kaggle pool, 1.000). {\bfseries Type B} is a content or modality confound, on which metadata may be silent (the Roboflow archive, 57/57 advisory graphics against 0.717). {\bfseries Type C} is one reintroduced when a {\itshape derived} set is drawn from a differently-acquired part of a clean source, which is how our own first synthetic proxy failed before use. {\bfseries Type D} is a shipped split whose partitions do not partition. {\bfseries Type E} is the audit's false-positive mode (the signature corpus, format 0.500 against size 0.843). Section S-I-T gives each its detector, its cost when undetected and its repair, and the two lessons cutting across them: publisher-side tidying suppresses the symptom rather than the disease, and the audit's output is not a verdict, since establishing that the separating statistic is an acquisition artifact rather than a property of the objects is a second step.
...
Table~\ref{tab:8} separates what this study demonstrates from what it hypothesizes or leaves unmeasured. The paragraphs after it state each limitation in turn, and Section S-II gives the evidence for and against each.
...
External acquisition shift exposes the failure & Demonstrated for authentic-class specificity: 97.4\% in-distribution against 6.0\% externally & Table~\ref{tab:4} \\
Normalization removes much of the acquisition signal & Demonstrated for M2 and M4; not demonstrated for M3 & Tables~\ref{tab:4} and \ref{tab:5} \\
The corrected models recognize packaging & Not established: a centre substitution costing neither backbone anything does not demonstrate packaging recognition & Table~\ref{tab:7} \\
The corrected models depend on the photographic surround & Demonstrated on both external sets: overwriting the outer half costs 22--50 points & Table~\ref{tab:7} \\
Correcting one provenance signal can expose another & Demonstrated on this dataset: the corrected models' recovered specificity localizes to the photographic surround, an acquisition property the audit does not test & Table~\ref{tab:7}, Section VII-E \\
...
{\bfseries The external evaluation is authentic-only}, so every external number is a specificity; the synthetic proxy is a corruption-robustness test in the spirit of ImageNet-C \cite{ref12}, never a recall measurement. Counterfeit sensitivity, counterfeit F1 and balanced accuracy under acquisition shift, and end-to-end authentication performance, are {\bfseries not measured anywhere in this study}. A degenerate classifier calling every image counterfeit would score 0.000 here and 1.000 on counterfeit recall, and nothing in our external data separates such a model from one applying a real decision rule at a shifted operating point. What the external sets establish is narrow: {\bfseries these models fail to recognize independently acquired authentic products as authentic} --- disqualifying for deployment, but not evidence that the counterfeit side survives the same shift.

{\bfseries No public counterfeit-labeled set was available to run that test}, and the reason is a property of what this area publishes rather than an experiment we neglected. Of the three archives inventoried (Section III-A), the one verified independent of the modeling pool carries no counterfeit label and became Split C. The one that does carry counterfeit labels is not external to that pool at all --- 42.3\% of the Kaggle pool has a near-duplicate in it (Section III-C); its counterfeit class is 57/57 advisory graphics with the ground-truth word rendered inside the image, leaving two usable counterfeit photographs (Section III-B); it documents no collection protocol or per-image provenance; and Section VI-B finds its acquisition traces already erased by publisher-side resizing and re-encoding, so it cannot be checked for acquisition balance either. Its labels are not a substitute for an external counterfeit test.

{\bfseries Both external sets share one backdrop.} They vary device and lighting on the same products, so the surround cue Section VI-F identifies is untested --- the study's most consequential gap, and it qualifies every claim that the correction "holds" across a capture shift.
...
{\bfseries The generality claim is a mechanism with a stated test, not a measured rate}, resting on seven audited datasets (Section S-I-W) and one convergent report \cite{ref30}; Section IX names the survey that would measure it.

{\bfseries Statistical power is thin throughout.} Test partitions hold 74--76 images, every pairwise comparison is underpowered, and the ablations and region substitutions are single executions, so a difference of a point or two should not be read as one. Table~\ref{tab:5} repeats the production and baseline conditions at five seeds; the ablations' own seed variance is unmeasured.
...
{\bfseries Acquire an external set that varies the photographic setting, and one that is counterfeit-labeled.} These are the two evaluations this study most needs and could not build. Photographs of the same packages against varied surfaces would test the surround cue of Section VI-F directly, at a fraction of the cost of either. The counterfeit-labeled set would change most; its requirements are specific --- independently photographed, verified by the pHash procedure of Section IV-B, and with acquisition balanced across classes so it does not import the confound it exists to test.
...
The consequence is severe. On 150 authentic photographs from an independent source, two of four models were correct on zero images and the best in-distribution model on nine of 150, while scoring 97.4\% on the authentic class of its own test partition. A label-free three-stage normalization raises external {\itshape authentic-class specificity} to 86.0\% and 80.7\% for two of the three, costing no more than 1.4 points in-distribution at seed 42 and $-$3.0 to +3.0 across five seeds --- and that is not the repair it looks like. Rephotographing the same packages on different hardware drops the from-scratch CNN to 0.627 $\pm$ 0.135 while the frozen backbones stay stable across that device and lighting shift; permuting only the operator order moves external specificity from 0.380 to 0.880 under a 2.7-point in-distribution range with the ranking inverted, so a practitioner tuning this preprocessing the ordinary way would have chosen the worst of six; and overwriting the outer half of the frame costs both corrected backbones 22 to 50 points while overwriting an area-matched centre region costs them nothing, which puts their "authentic" verdicts on the photographic surround rather than the packaging. It is not a remedy, for the paper's most transferable reason: {\bfseries eliminating one provenance signal can expose another}, so removing a confound from the input is itself dataset construction and needs auditing on the same terms.
...
Two qualifications come with it. The audit is necessary but not sufficient: on a second pharmaceutical dataset it returned 0.717 while that dataset was confounded at least as badly, its publisher having normalized the acquisition traces away without removing the confound. And it says nothing about whether a model generalizes; only data someone else acquired does that. That the same signature is documented independently in generated-image detection \cite{ref30}, and appears in a third archive in our own pilot audit, is what a structural cause predicts --- a mechanism we would expect wherever a scarce class must be manufactured or harvested separately from an abundant one, and not a rate we have measured.
...
{\bfseries Repository.} All code and derived artifacts are at \url{https://github.com/sophiezla/counterfeit-drug}, archived at the concept DOI {\bfseries 10.5281/zenodo.21936720}, which resolves to the most recent release. The release accompanying this manuscript is {\bfseries v1.3.1}, archived at doi:10.5281/zenodo.22239515, and it is the exact state of the code that produced every number reported here; \texttt{README.md} and \texttt{CITATION.cff} name it, and the earlier releases they list are marked superseded. It holds the data pipeline, the four model implementations, every analysis and figure script, the per-image statistics and split assignments, the persisted checkpoints, and the sources of this manuscript and its supplement.
