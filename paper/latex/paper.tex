...
{\bfseries Two independent lines of evidence support reading it that way.} Running the audit of Section III-E on seven public archives across four application areas, we find the same total confound --- container format alone reproducing the label --- arising in a {\bfseries generated-image} dataset built by different people for a different task, while a signature corpus whose two classes were written on one paper and digitized by one procedure scores at chance on the same axis (Table~\ref{tab:3}). Independently, Grommelt {\itshape et al.} \cite{ref30} report the mechanism in generated-image detection from the other direction: real images are harvested as lossy JPEGs at modest resolution while generated images are written as lossless PNGs at native size, which on GenImage makes format, compression and size predictive of the label, partly reduces detectors to JPEG detectors, and shifts cross-generator performance by more than 11 points once those factors are equalized.

We separate three levels of evidence and hold to them throughout. {\bfseries Demonstrated}: the case-study dataset is completely provenance-confounded, container format reproducing the label on all 510 images (Section IV-A). {\bfseries Supported}: the mechanism arises outside this dataset and outside this application area, on the two independent lines just given. {\bfseries Not established}: how frequently it occurs. The seven archives were chosen for accessibility and have no sampling frame, so they demonstrate that the audit discriminates across domains and cannot estimate a rate; that would require auditing a representative sample, which this paper does not attempt. Table~\ref{tab:8} records every claim against these three levels, and Section V-B states what would count against the mechanism.
...
The result was not the one the design anticipated. Correcting the split changed in-distribution accuracy by at most 6.8 points; external validation changed the picture. On 150 authentic photographs from an independent source, two of four models classified {\itshape zero} correctly and the strongest in-distribution model nine. That model reaches 97.4\% authentic-class accuracy on its own internal test partition; the two figures come from separate evaluations on different images, not from a paired measurement. Looking for an explanation led to the dataset itself: its two classes were not merely photographed differently but {\itshape acquired} differently, every counterfeit-labeled file being a screen capture (\texttt{Screenshot*.png}) and every authentic-labeled file a JPEG photograph (\texttt{images*.jpg}), correlating with the label exactly 1.0 across the pool. Any model, including a 97-parameter linear classifier on a color histogram, has unobstructed access to it.

There is one contribution and three findings, and they are not of equal weight.

{\bfseries Primary contribution: a pre-training provenance audit}, stated as a procedure in Section III-E. It needs no external data, no annotation, no training run and no pixel decoding, and returns {\bfseries 1.000} here from header fields alone. A high score establishes that a provenance shortcut is {\itshape available}, not that a given pixel classifier took it. We also report where it fails: on a second published dataset it returns only 0.717 although that dataset is confounded at least as severely, because its publisher had normalized every image before release, so that a tidied dataset is {\itshape harder} to audit than a raw one; and one archive exposes a false-positive mode on the mixed axes (Table~\ref{tab:3}).

{\bfseries Primary empirical finding: the case-study dataset carries a deterministic class--acquisition confound, and it invalidates in-distribution performance as evidence of authentication ability.} We report a confound the dataset's listing does not disclose and that no study located by our search reports, together with a degenerate shipped split whose training folder is a superset of both others (Sections III-A, IV-A). No accuracy measured on that pool separates packaging recognition from provenance recognition.

{\bfseries Secondary finding: external testing exposes the failure; the leakage correction this literature emphasizes does not.} Re-partitioning the pool at the near-duplicate-cluster level moves accuracy by at most 6.8 points, which is a descriptive comparison of two split {\itshape designs} differing in their test images as well as in their leakage, not an estimate of leakage itself; the controlled estimate holds the test set fixed and varies only whether near-duplicate mates are admitted, and moves accuracy by 0.3 points [$-$1.9, +2.4]. Both are small beside what external data shows. On an {\bfseries independent balanced set of 46 authentic against 46 counterfeit products}, assembled from two unrelated archives, with container format, encoder, encoding settings and stored resolution standardized across the classes at ingestion, and scored once against frozen checkpoints, the four models return balanced accuracies of 0.478, 0.478, 0.543 and 0.652 --- only the last excluding chance --- and the 97-parameter baseline is exposed as a degenerate all-counterfeit classifier that an authentic-only evaluation could not have distinguished from a working one. Under a capture-condition shift the same models recover as little as 6.0\% of plainly authentic photographs (Sections IV-B--IV-D).

{\bfseries Exploratory finding: removing the acquisition statistics the audit names does not remove every provenance shortcut.} A label-free normalization raises external authentic-class specificity from 6.0\% to 81--86\% for two of the three models it is applied to, the third's apparent gain lying inside seed variance. We decline to offer it for use. Region substitution shows that both corrected backbones then rely strongly on the outer region of the frame rather than on the center where the packaging sits, so the correction removes the confound the audit found and leaves the models depending on a signal consistent with acquisition-linked cues the audit does not test (Section IV-E). Provenance auditing is on this evidence better treated as an iterative dataset-quality check than as a one-time preprocessing step.
...
{\bfseries Second, the most common class-construction procedures make the confound likely.} In \cite{ref26} the counterfeit class is produced by digitally editing authentic images and in \cite{ref27} by a generative model, so both are by construction outputs of two different image pipelines, as here. Where the classes {\itshape were} acquired under a common protocol --- \cite{ref3}, on one Raspberry Pi rig --- the reported accuracy is the lowest in Table S26 (88.75\%). We note the direction and stop there, the studies differing in dataset, task framing, architecture and sample size. Outside pharmaceuticals, Garcia-Cotte {\itshape et al.} \cite{ref29} report counterfeit detection on branded garments captured "under natural, weakly controlled conditions", stating the acquisition regime as a property of the result --- the reporting standard Section V-C argues should become routine here.

\subsection{Shortcut learning and dataset bias}

Geirhos {\itshape et al.} \cite{ref6} formalized {\itshape shortcut learning}: networks adopt decision rules exploiting superficial, spuriously predictive correlations, scoring well in-distribution while failing wherever the shortcut is absent. The failure has a documented precedent in medical imaging. Zech {\itshape et al.} \cite{ref7} showed that pneumonia detectors trained on chest radiographs from three hospital systems could predict which system an image came from, and used that hospital-identity signal --- itself correlated with disease prevalence --- as a shortcut for the diagnostic label, degrading substantially on an unseen site. Later audits report the same for scanner-, site- and manufacturer-level signal \cite{ref8}, \cite{ref9}, and early COVID-19 radiograph classifiers were shown to rely on dataset-source confounds rather than radiographic signs \cite{ref10}. DeGrave {\itshape et al.} \cite{ref31} put the case most sharply: applying explainable-AI methods to published COVID-19 radiograph detectors, they conclude the systems rely on confounding factors rather than pathology, and so "appear accurate, but fail when tested in new hospitals".

\subsection{What this paper adds}

The contribution is not the discovery that models learn shortcuts, and it is worth separating from the terms nearest to it. {\itshape Shortcut learning} \cite{ref6} names what a model does. {\itshape Dataset bias} \cite{ref32} and {\itshape spurious correlation} name a statistical property of a sample. {\itshape Domain shift} names a relation between two distributions, one of them the deployment distribution. {\itshape Hidden stratification} \cite{ref7} names an unlabeled subgroup structure inside a class. {\itshape Source confounding} names an association between data origin and label. {\itshape Metadata leakage} names label-predictive information carried outside the intended input. {\itshape Near-duplicate leakage} \cite{ref11} names a partitioning fault, in which images of one entity fall on both sides of a split; that concern motivated this work's two-split design, and Section IV-B measures its size rather than assuming it. {\itshape External validation} names an evaluation practice.

Class-conditional provenance confounding rivals none of these. It is a {\itshape dataset-construction mechanism} that can produce several of them at once, named separately because the cause is what makes it detectable in advance: because one class was unobtainable by the procedure that produced the other, acquisition is correlated with the label {\itshape within the training distribution itself}, before any model is fitted and without reference to any deployment distribution. Three consequences follow that the general terms do not carry. The association can be complete rather than partial, incidental at the three hospital systems of \cite{ref7} and exactly 1.0 across 510 images here. It is predictable from a dataset's construction, so the population at risk is nameable in advance: any collection whose classes were sourced separately. And it survives leakage-aware validation, every partition of one pool inheriting it in the same proportion.

What we offer beyond these is procedural, for the reason Section I-A gives: the diagnosis reduces to a screen that reads file listings before any model exists, and we follow the correction through to show that removing one provenance signal can expose another. We did not identify prior work applying such an audit procedure to counterfeit-medicine image authentication, or reporting it as a pre-training step rather than as a post-hoc explanation. The closest published analogue outside medicine is \cite{ref30}, described in Section I-A. Domain-generalization surveys \cite{ref23} situate the normalization of Section III-F closer to hand-designed covariate-shift alignment than to representation learning, and the synthetic-corruption proxy of Section S-I-B follows the logic of ImageNet-C \cite{ref12}, measuring robustness to a documented perturbation style rather than label-defined class recall.
...
Table S2 gives every training setting in one place, and nothing in it varies by split, condition or experiment: Adam at 1 $\times$ 10\textsuperscript{-}\textsuperscript{3} for M2--M4, selected from \{1 $\times$ 10\textsuperscript{-}\textsuperscript{3}, 3 $\times$ 10\textsuperscript{-}\textsuperscript{4}, 1 $\times$ 10\textsuperscript{-}\textsuperscript{4}\} on Split A train/val alone; batch 32; class-weighted cross-entropy; a 50-epoch cap with early stopping on validation loss at patience 4 and minimum improvement 1 $\times$ 10\textsuperscript{-}\textsuperscript{3}; best-validation-loss state persisted as a checkpoint; a 0.5 decision threshold; and seed 42 fixed before every run, fold and augmented extraction pass, with sensitivity at 42--46 (Table~\ref{tab:6}). M1 is fitted by \url{LogisticRegression(max_iter=2000, class_weight="balanced")} with neither augmentation nor the normalization operator. All runs are CPU-only.

\subsection{External evaluation}

Throughout, authentic = 0 and {\bfseries counterfeit is the positive class}, so precision, recall, F1 score and the areas under the receiver operating characteristic and precision--recall curves are reported with respect to counterfeit detection --- the deployment framing, in which the costly error is calling a falsified product genuine. Because a reader in a clinical setting may take "positive" to mean {\itshape genuine}, the convention is set out in full here and every later use follows it.

\begin{center}
\footnotesize
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.15}
\begin{tabularx}{\columnwidth}{>{\hsize=0.432\hsize\raggedright\arraybackslash\hyphenpenalty=10000\exhyphenpenalty=10000}X>{\hsize=1.568\hsize\raggedright\arraybackslash\hyphenpenalty=10000\exhyphenpenalty=10000}X}
\toprule
...
Accuracy & Fraction correct over a {\bfseries mixed-class} set: the internal grouped test, and the balanced external test \\
External specificity & Specificity on an {\bfseries authentic-only} set; the only quantity such a set can measure \\
External counterfeit recall & Recall on a {\bfseries counterfeit-only} source; the only quantity such a source can measure \\
\bottomrule
\end{tabularx}
\end{center}
...
{\bfseries The authentic class could not come from any source already in this study, and saying why is the design.} The regulatory archives supply one: of nine authentic-labeled candidates, eight are flat vector carton artwork against field photographs for every falsified one --- a {\bfseries +0.233 mean-brightness gap} that is a property of how the images were produced and not of the products. A regulator publishes photographs of what it seized and manufacturer artwork of what the genuine article should look like, so that asymmetry is structural rather than an accident of one harvest. The acquisition-shift conditions supply the other: pairing their photographs with the regulatory ones would open a brightness gap of {\bfseries +0.396} and set a 2448 px median short side against 439 px --- {\itshape larger} than the gap that disqualified the artwork. Either route would have reproduced the confound this paper exists to expose inside the paper's own external validation.

{\bfseries So the authentic class was sourced independently}, from Wikimedia Commons --- a third-party photographic archive with no relation to any source here, carrying per-file license and authorship --- and product-matched to the 46 alerts, whose titles name their products. Of 3,568 candidates, 788 survive a pre-download filter on title and native resolution and 672 were fetched; automated screens on flatness, brightness and duplication leave 449; a rotation-canonical pHash check against all 7,283 images under \texttt{data/raw} returns {\bfseries 0 matches within the threshold}, closest approach Hamming 10 --- the same closest approach Split C and the regulatory candidates each recorded --- and removing 11 within-set near-duplicates leaves 438. A recorded subject-matter review (Section S-IX) then accepts {\bfseries 174}: Commons, searched by drug name, returns a great deal that is not a photograph of a medicine package, and 99 candidates show no medical product at all, 69 make a person the subject, 38 are drawings or renderings, 28 are museum objects and 25 show a loose dose form with no packaging. The final 46 are chosen from the 174 by nearest-neighbour brightness matching to the 46 counterfeit frames, at most one per product search term while the terms last, which spreads them over {\bfseries 29 distinct products}.
...
{\bfseries What the experiment can and cannot measure.} Both conditions hold authentic images only, so every figure is a specificity and {\bfseries the experiment measures no counterfeit recall whatever}. Because content is held approximately fixed while capture varies, what a model surviving C $\rightarrow$ D is shown to be is stable across {\bfseries the device and lighting shift these two conditions represent} --- not across acquisition in general, the two conditions sharing one archive, one set of packages and one dark backdrop. The correspondence is at the level of the set rather than the image, so the two are compared as independent proportions and {\bfseries the 299 images are not pooled into one sample}: the same products appear in both, and treating them as 299 independent observations would overstate the precision of anything computed across the pair.
...
{\bfseries Occlusion sensitivity} is annotation-free and runs over every external image, producing the fraction of decision-relevant mass falling in a border ring against a 0.642 uniform reference. {\bfseries Grad-CAM} \cite{ref15}, for target class $c$ with $A^k$ the activation maps of the last convolutional stage,
...
is treated as secondary and corroborating evidence only: its categorization covers 62 maps placed in one of three categories by a single annotator, with no inter-rater agreement and no rubric fixed in advance, so it is not a reproducible measurement (Section V-D). We call these {\itshape attribution} rather than {\itshape attention} analyses throughout, since neither is an attention mechanism.
...
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.15}
\begin{tabularx}{\columnwidth}{>{\hsize=0.538\hsize\raggedright\arraybackslash\hyphenpenalty=10000\exhyphenpenalty=10000}X>{\hsize=1.832\hsize\raggedright\arraybackslash\hyphenpenalty=10000\exhyphenpenalty=10000}X>{\hsize=0.630\hsize\raggedright\arraybackslash\hyphenpenalty=10000\exhyphenpenalty=10000}X}
\toprule
{\bfseries Tier} & {\bfseries What it covers} & {\bfseries Where} \\
\midrule
{\bfseries Confirmatory} (pre-specified) & The provenance audit; the Split A vs. Split B comparison and the paired leakage experiment; the baseline external evaluation of the four models on condition C & Sections IV-A, IV-B, and the baseline column of Table~\ref{tab:5} \\
{\bfseries Protocol-fixed} (frozen before the data existed) & The balanced external evaluation: model, preprocessing, normalization, threshold and metric set were all frozen before the set was built, and it was scored once & Section IV-C, Table~\ref{tab:4} \\
{\bfseries Exploratory} (post-hoc) & The normalization and the selection of its axes, constants and order; the attribution and region-substitution investigations; condition D & Sections IV-D, IV-E \\
\bottomrule
\end{tabularx}
\end{center}


The second row is labeled {\bfseries protocol-fixed} rather than confirmatory, and the distinction is deliberate. The {\itshape decision} to build a balanced external set was taken after the one-sided results were in, so it was {\bfseries not pre-registered} in the sense the first row is. What it does have is a frozen protocol: the models, their preprocessing, the normalization, the decision threshold and the metric set were all settled before the set was built, and the set was scored once (Section III-D).

The third tier is where the strongest-looking numbers in this paper live, and it is also where condition C stops being an untouched external set: the three normalization axes were nominated {\itshape because} Table~\ref{tab:1} showed those statistics separating the training pool from it (Section III-F). The 86\% and 81\% figures of Section IV-E are therefore not clean external-validation results, and we do not report them as such.

\subsection{The provenance audit}

The strongest result in this study needs four learned parameters and no decoded pixels: a logistic regression on three header fields separates the two classes perfectly on the leakage-free partition (Table~\ref{tab:2}).

Every one of the 510 pool filenames falls into exactly one of two patterns, and {\bfseries the pattern predicts the class label with no exceptions}: 272/272 authentic-labeled files are \texttt{images*.jpg} and 238/238 counterfeit-labeled files \texttt{Screenshot*.png}. This is not an artifact of our filtering --- it holds in the archive as distributed, where all 240 files in \texttt{Fake/} are \texttt{Screenshot*.png} and all 421 in \texttt{Real/} are \texttt{images*.jpg} --- so any study using this dataset inherits it in full, and a classifier reading nothing but the file extension achieves {\bfseries 100\% accuracy}.
...
The confound is visible in the simplest model's decision function. Of M1's 96 coefficients, 93 lie within $\pm$0.35 of zero while the top intensity bin (248--255) of each channel carries a large negative weight ($\beta$ = $-$2.86, $-$2.84, $-$2.95), and the logit decomposition of \eqref{eq:10} puts those three bins in the top three positions by mean |$\varphi$|, at 0.079--0.082 against 0.002 for the fourth (Section S-I-P). M1's 83.8\% accuracy therefore appears driven largely by near-white pixel prevalence rather than by packaging.
...
{\bfseries Table~\ref{tab:3} is an opportunistic feasibility screen, not a survey.} The archives were selected for public accessibility rather than by any sampling frame. It is reported to establish that the procedure runs cheaply against unfamiliar collections and to characterize its sensitivity and its failure modes. {\bfseries No estimate of how often provenance confounding occurs in image datasets can be drawn from it, and none is claimed.}

{\bfseries Within those limits, the screen discriminates.} Two archives return 1.000 on format and two at or near chance. The generated-image pair is the sharper case: two datasets built for the same task by different people return 1.000 and 0.577, which cautions against reading the mechanism as a law.
...
{\bfseries Three of the four models are indistinguishable from chance, and one of the three is below it.} Balanced accuracy runs 0.478, 0.478, 0.543 and 0.652, and {\bfseries only M4's interval excludes 0.500}. M1 and M2 return ROC-AUCs of 0.422 and 0.403, whose intervals reach above 0.5 but whose point estimates are on the wrong side of it: on this set their scores carry no usable information about the label, and what ordering they do impose runs backwards. These are the same four models that reach 0.842 to 0.987 on their own internal partitions.
...
{\bfseries Surviving the acquisition-shift test does not predict this result, and M2 is the demonstration.} M2 holds the highest external specificity anywhere in this study --- 0.860 on condition C, above its own in-distribution authentic-class accuracy (Table~\ref{tab:5}) --- and returns balanced accuracy 0.478 with ROC-AUC 0.403 on the balanced set. The ordering of the four models is not preserved between the two evaluations either: M2 leads on condition C and is joint-last here, tied with the 97-parameter baseline, while M4 leads here. {\bfseries The two evaluations answer different questions, and the one-sided answer does not transfer.}

{\bfseries What Table~\ref{tab:4} does and does not establish.} It establishes that these models, evaluated on independently sourced photographs of both classes with container format, encoder, encoding settings and stored resolution standardized across the classes by construction, perform at or near chance, with the single best reaching 0.652 [0.554, 0.750]. It does {\bfseries not} establish general counterfeit-detection performance in the field, and the shift here is one of source rather than of the acquisition axis this paper is about; Section V-D states the bound and what the two classes are samples of.
...
\begin{table*}[!t]
\caption{The acquisition-shift experiment: authentic-class specificity under two capture conditions on approximately the same products. Every figure is a true-negative rate and none estimates counterfeit recall. {\bfseries Both model conditions come from the current deterministic pipeline at seed 42}, so the baseline and normalized columns differ only by the operator of \eqref{eq:8} and the comparison is within-run; Table~\ref{tab:6} gives both across five seeds. The in-distribution reference is authentic-class accuracy on each model's own Split B test partition (n = 39). Conditions C and D cover the same packages and are compared as independent proportions rather than pooled (Section III-D). Intervals are 95\% Wilson on the counts given as k/n.}
\label{tab:5}
\centering
\footnotesize
\setlength{\tabcolsep}{3pt}
...
\begin{table*}[!t]
\caption{Five-seed sensitivity analysis, seeds 42--46, mean $\pm$ sample standard deviation. Every other number in this paper comes from one run at seed 42; this table says how much of it is initialization. Split assignments, preprocessing and learning rates are fixed across seeds, so the training seed is the only thing that moves. Both conditions come from the same current harness as Table~\ref{tab:5}, whose columns are this table's seed-42 draw. Conditions C and D are authentic-only, so those columns are specificities. M1 is omitted: a convex fit with a deterministic solver and no augmentation, its accuracy is identical under every seed. Per-seed values are in Section S-I-U.}
\label{tab:6}
\centering
\footnotesize
\setlength{\tabcolsep}{4pt}
...
\begin{tabular}{lcccc}
\toprule
{\bfseries Model} & {\bfseries Condition} & {\bfseries Split B test} & {\bfseries Condition C} & {\bfseries Condition D} \\
\midrule
...
{\bfseries The regulatory source in its native encodings.} All 150 regulatory photographs, rather than the 46 that enter Table~\ref{tab:4}, were also scored in their original files. That measurement is a one-sided recall under a source shift and is reported in Section S-IX with its sensitivity analysis, not here: it cannot be read beside a specificity without the reasoning Section IV-C now supplies directly, and Table~\ref{tab:4} supersedes it as the counterfeit-side evidence of record.
...
The normalized columns of Table~\ref{tab:5} carry the qualification stated at the head of this section: {\bfseries condition C was not an untouched validation set for this experiment} (Section III-F).

The same models, retrained on the same images with the same seeds and hyperparameters and differing only by the label-free operator of \eqref{eq:8}, recover 86.0\%, 77.3\% and 80.7\% external authentic-class specificity. Because both conditions are evaluated on the same 150 images, the comparison is paired; Section S-I-Z runs it across all five seeds, resampling images so both arms' verdicts move together, and reports {\bfseries +85.9 points [+81.7, +89.7]} for M2, {\bfseries +76.0 [+69.9, +81.5]} for M4, and {\bfseries +0.9 [$-$6.9, +8.8]} for M3 (Table S23). M3's baseline and normalized seeds interleave (0.715 $\pm$ 0.057 against 0.724 $\pm$ 0.047), so {\bfseries we claim no normalization benefit for M3}, as a measurement rather than a caution, and rest the result on M2 and M4.

{\bfseries The in-distribution price is small at the run of record and larger across seeds.} At seed 42 it is small --- no more than 1.4 points in either direction --- but that describes the run of record, not the five-seed record, which is larger and signed both ways: M2 loses 3.0 points and M4 gains 3.0 (Table~\ref{tab:6}). M2's external specificity exceeds its own in-distribution accuracy, the only negative gap here, because with the shortcut suppressed its mixed-class test partition is the harder problem. M1 bypasses the operator and is unchanged at 0.000.

{\bfseries The recovery is not an artifact of having seen the external set, and it is also not a remedy.} Section S-I-S re-derives the three axes inside the Split B training partition alone under a threshold fixed in advance, and Table S27 runs the operator that rule nominates end to end for all three models and both authentic-only external sets, with nothing in its construction consulting an external image; it averages 0.842 against the reported operator's 0.818. A practitioner who never saw condition C would therefore have measured the same effect, and slightly more of it.

Sections S-I-N to S-I-X ablate the correction per axis, per architecture, per constant and per composition order. One of those results bears on how any such pipeline should be reported: {\bfseries composition order matters more than any of the three magnitudes}, and the ordering with the highest in-distribution accuracy of the six has the lowest external specificity of the six, so a practitioner selecting the order the ordinary way would have chosen the worst while watching a small in-distribution spread conceal a large external one (Section S-I-X). Composition order is normally left implicit in a preprocessing description; on this evidence it deserves the treatment of a hyperparameter.

{\bfseries We do not propose \eqref{eq:8} for adoption.} It is an instrument for testing whether intervening on three named acquisition statistics changes the external result, and it does. What it does not do is establish that the corrected models recognize packaging, which the next analysis takes up.
...
{\bfseries What this does and does not establish.} It demonstrates that both corrected backbones depend strongly on the {\bfseries outer region of the frame} rather than on the center where the packaging sits. It does not isolate {\itshape which} property of that region they use. The outer region holds the dark backdrop, the lighting and shadow gradient, the staging, and also whatever of the carton extends past the center square, and a geometric partition cannot separate these. Establishing that the backdrop specifically is the operative cue would need content-aware annotation --- attribution mass inside an annotated product box rather than inside a fixed geometric region --- which we have implemented but not run, as it requires an annotation pass with rotated boxes (Section V-C). We therefore describe the finding as outer-region dependence, and treat backdrop, lighting and staging as the candidate cues it is consistent with.
...
One further result cautions against a common practice. {\bfseries A visually convincing, product-centered Grad-CAM map is not evidence that a model learned the intended task.} That rests on a quantitative mass statistic rather than on the categorization just set aside: for the un-normalized model, whose input is unaltered, 0.563 of the attribution mass falls inside a center box covering 0.161 of the frame, and that model classifies almost no external image correctly (Table~\ref{tab:5}). Across the two conditions, spatial concentration of attribution {\itshape anti-correlates} with external specificity.
...
External behavior was not monotonic with model capacity across the evaluated configurations, which suggests that access to acquisition-related features, and the specific distribution shifts tested, were more consequential than parameter count alone. M1's feature space {\itshape is} the shortcut, so the correction unlocks nothing for it, while the two frozen backbones inherit ImageNet invariances that appear to afford partial protection (Section S-I-Q). We do not press this further: with four architectures, one dataset and the shifts tested here, it is a reading consistent with the results rather than something the design can establish.

\subsection{Why provenance auditing matters, and what it does not settle}

The normalization of \eqref{eq:8} does what it was designed to do, and its own results are why we report it as an instrument rather than a remedy. The recovered specificity rests on the outer region of the frame rather than the packaging; the exchange is not stable, the best corrected model on condition C falling below its own level on a second capture of the same products; the pipeline is under-determined by its own description, since permuting operator order alone moves the external result far more than the in-distribution result while inverting the ranking; and tightening the resolution bottleneck further keeps helping, which is what a coarse distribution match predicts and not what restored reading of printed detail predicts.

The axes were chosen because Table~\ref{tab:1} showed them separating the pool from condition C, so intervening on them might seem guaranteed to help. It is not: a bottleneck that removed the discrepancy while destroying the signal would have driven in-distribution accuracy to chance, as it does for M1. What the experiment supports is an {\itshape interventional} claim about three named statistics and nothing wider. It does not identify which of the three, or what within them, the models were responding to, the axes being correlated and removed together, and it is not evidence that they learned to recognize packaging.
...
\begin{itemize}
\item {\bfseries Acquisition-source balance.} Every capture condition --- rig, camera, operator, location, lighting --- must contribute {\itshape both} classes. If authentic items are photographed on rig A, counterfeit items must be photographed on rig A too, and vice versa. A class obtained by a different route than the other is the mechanism this paper describes.
...
\item {\bfseries Report the audit score before the model score.} Publish the $A_{\mathrm{pure}}$ audit result --- container format at minimum --- alongside baseline performance, and state the partition it was computed on. A dataset card that reports 1.000 is more useful than one that reports nothing.
\item {\bfseries Ship a raw archive, and document the collection protocol.} Publisher-side resizing and re-encoding erase the traces a cheap audit reads without removing the confound beneath them, which is how one archive audited here scores at chance while being confounded at least as severely as the case study.
...
External acquisition shift exposes the failure & Demonstrated for authentic-class specificity: the model reaching 97.4\% authentic-class accuracy on its own internal test partition returns 6.0\% authentic-class specificity under the acquisition shift; the two are separate evaluations, not a paired measurement & Table~\ref{tab:5} \\
Normalization removes much of the acquisition signal & Demonstrated for M2 and M4; not demonstrated for M3 & Tables~\ref{tab:5} and \ref{tab:6} \\
The corrected models recognize packaging & Not established: a center substitution costing neither backbone anything does not demonstrate packaging recognition & Table~\ref{tab:7} \\
The corrected models depend on the photographic surround & Demonstrated in both acquisition-shift conditions: overwriting the outer half costs 22--50 points & Table~\ref{tab:7} \\
Correcting one provenance signal can expose another & Demonstrated on this dataset: the corrected models' recovered specificity localizes to the photographic surround, an acquisition property the audit does not test & Table~\ref{tab:7}, Section V-B \\
...
Two-class performance on an independent balanced set & Demonstrated: balanced accuracy 0.478, 0.478, 0.543 and 0.652 on 46 authentic against 46 counterfeit, only M4's interval excluding chance; ROC-AUC 0.403 to 0.713 & Table~\ref{tab:4} \\
One of the four models is the degenerate all-counterfeit classifier & Demonstrated: M1 calls 90 of 92 images counterfeit, for 0.957 recall at 0.000 specificity and 0.478 balanced accuracy & Table~\ref{tab:4} \\
...
{\bfseries The two external evaluations do not cover the same axis, and neither covers the one that matters most.} The balanced test measures both directions at once but shifts {\itshape source} while standardizing file-level encoding conditions by construction; the acquisition-shift experiment moves the acquisition axis but holds one class only. {\bfseries Two-class performance under acquisition shift is therefore measured nowhere in this study}, and neither is end-to-end authentication performance. The synthetic proxy is a corruption-robustness test in the spirit of ImageNet-C \cite{ref12}, never a recall measurement.
...
{\bfseries The counterfeit side's eligibility screen is one reviewer's single pass}, excluding 52 of 202 candidates on per-frame judgements whose codes correlate with class by construction. Table S28 bounds the effect at 0.06 of recall on the full 150-image source, but a bound is not an inter-rater agreement and none was measured; the screen is published per image so it can be overruled, as above.

{\bfseries "Leakage-free" means near-duplicate leakage only}, at Hamming distance $\le$ 8 (Section S-I-Y). No partition of this pool can decorrelate acquisition, because the counterfeit class exists in exactly one capture pipeline; cross-validation across sources fails for the same reason.
...
The methodological claim we would most like carried forward is cheap to act on. Before training anything, fit the classifier you intend to use to acquisition variables alone --- format, encoded size, resolution, aspect ratio --- under your own leakage-free split, and report the number, reading attributes the photographed object cannot influence separately from those that mix acquisition with content. It is a diagnostic on the dataset: it tells you whether a provenance shortcut is {\itshape available}, not whether your model takes it. But where it is high, no in-distribution accuracy on that dataset can distinguish the two. Here it is 1.000. The audit is necessary and not sufficient: on a second pharmaceutical dataset it returned a middling score while that dataset was confounded at least as badly, its publisher having normalized the acquisition traces away without removing the confound.
