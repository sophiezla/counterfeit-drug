...
[v1.4.0](https://github.com/sophiezla/counterfeit-drug/releases/tag/v1.4.0)
(archived on Zenodo under the concept DOI above, which resolves to it; the
version DOI is recorded here once minted), which is the exact
...
## Status (2026-09-13): submitted state

The manuscript (`paper/PharmaChecked_v2_manuscript_IEEEAccess.pdf`, 22 pages)
...
- **The provenance audit** (Section III-E) returns 1.000 on the case-study
  pool: container format alone reproduces the label on all 510 images.
- **Three evaluations**, named for what they measure: the internal grouped
  test (Split B), the **balanced external test** (46 regulator-confirmed
  counterfeits from FDA/WHO alerts against 46 authentic-labeled Wikimedia
  Commons photographs, one ingestion pipeline, scored once; Table 4), and the
  **external acquisition-shift test** (conditions C and D of the Mendeley
  archive, authentic-only; Table 5).
- **Results:** balanced accuracy 0.478 / 0.478 / 0.543 / 0.652 for M1-M4 on
  the balanced set, only M4's interval excluding chance, and M1 measured as
  the degenerate all-counterfeit classifier; under the acquisition shift the
  model reaching 97.4% authentic-class accuracy in-distribution recovers 6.0%
  of 150 authentic photographs. An exploratory normalization recovers most of
  that specificity for two of three models and leaves both corrected
  backbones depending on the outer region of the frame (Table 7).
- **Nothing here is a deployable authenticator**, and the paper says so.

The sections below this one are the project's **working log from July
2026**, kept as a record of how the finding was reached. Where a number
there disagrees with the manuscript, the manuscript is right: the log
predates the checkpoint and seeding fixes (Section S-I-G), the regulatory
counterfeit source and the balanced test, and it uses "Split C accuracy" for
what the paper calls external specificity.
