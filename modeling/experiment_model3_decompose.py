"""
Follow-up to Finding 6 (data/metadata/capture_method_confound_findings.md):
combined brightness+resolution normalization HURT Model 3 (MobileNetV3-
Small)'s Split C accuracy (73.3% -> 52.0%), the opposite direction from
Models 2 and 4. This decomposes the combined effect into its two
components for Model 3 specifically (mirroring the 4-condition breakdown
already done for Model 4 in experiment_resolution_norm.py) to find out
whether resolution normalization, brightness normalization, or both are
responsible for the regression.

Output: modeling/results/model3_decompose_experiment.csv
"""
import csv
...
OUT_PATH = Path(__file__).resolve().parent / "results" / "model3_decompose_experiment.csv"


def run_model3(norm_resolution: bool, norm_brightness: bool, run_tag: str):
...
def main():
    ...
    for name, res_norm, bright_norm in conditions:
        print(f"=== Model 3 (MobileNetV3-Small): {name} ===")
        acc, c_acc = run_model3(res_norm, bright_norm, run_tag=f"m3decomp_{name}")
