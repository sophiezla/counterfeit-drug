...
Correcting one provenance signal can expose another & Demonstrated on this dataset: the corrected models' recovered specificity localizes to the outer region of the frame, an acquisition-linked property the audit does not test & Table~\ref{tab:7}, Section V-B \\
...
{\bfseries The two acquisition-shift conditions share one backdrop.} They vary device and lighting on the same products, so the outer-region cue of Section IV-E is untested, which qualifies every claim that the correction "holds" across a capture shift.
